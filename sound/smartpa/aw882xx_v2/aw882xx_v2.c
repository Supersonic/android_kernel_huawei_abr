...
#define AW882XX_I2C_NAME "aw882xx_smartpa_v2"
...
int aw882xx_i2c_write_v2(struct aw882xx *aw882xx,
...
int aw882xx_i2c_read_v2(struct aw882xx *aw882xx,
	unsigned char reg_addr, unsigned int *reg_data)
{
	int ret = -1;
	unsigned char cnt = 0;
	...
	unsigned char reg_addr, unsigned int mask, unsigned int reg_data)
{
	...
	ret = aw882xx_i2c_read_v2(aw882xx, reg_addr, &reg_val);
	if (ret < 0) {
	...
	reg_val &= mask;
	reg_val |= reg_data;
	ret = aw882xx_i2c_write_v2(aw882xx, reg_addr, reg_val);
	if (ret < 0) {
		aw_dev_err(aw882xx->dev, "i2c read error, ret=%d", ret);
		return ret;
	}
...
int aw882xx_hw_reset_v2(struct aw882xx *aw882xx)
{
	aw_dev_info(aw882xx->dev, "enter");
...
static int aw882xx_read_chipid(struct aw882xx *aw882xx)
{
	...
	while (cnt < AW_READ_CHIPID_RETRIES) {
		ret = aw882xx_i2c_read_v2(aw882xx, AW882XX_CHIP_ID_REG, &reg_value);
		if (ret < 0) {
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (2 == sscanf(buf, "%x %x", &databuf[0], &databuf[1]))
		aw882xx_i2c_write_v2(aw882xx, databuf[0], databuf[1]);
	...
	struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < reg_num; i++) {
		if (aw882xx->aw_pa->ops.aw_check_rd_access(i)) {
			aw882xx_i2c_read_v2(aw882xx, i, &reg_val);
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (aw882xx->aw_pa->ops.aw_check_rd_access(aw882xx->rw_reg_addr)) {
		aw882xx_i2c_read_v2(aw882xx, aw882xx->rw_reg_addr, &reg_val);
		len += snprintf(buf+len, PAGE_SIZE-len,
...
static struct of_device_id aw882xx_dt_match[] = {
	{ .compatible = "awinic,aw882xx_smartpa_v2" },
	{ },
};
...
static int __init aw882xx_i2c_v2_init(void)
{
	int ret = -1;
...
module_init(aw882xx_i2c_v2_init);


static void __exit aw882xx_i2c_v2_exit(void)
{
	i2c_del_driver(&aw882xx_i2c_driver);
}
module_exit(aw882xx_i2c_v2_exit);
