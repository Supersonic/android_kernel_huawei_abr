...
#include "aw882xx_v2.h"
#include "aw_dsp_v2.h"
#include "aw_log_v2.h"
#include "aw_calib_v2.h"
#include "aw_device_v2.h"
#include "aw_data_type_v2.h"
...
static const char *cali_str[CALI_STR_MAX] = {"none", "start_cali", "cali_re",
	...
	"show_te", "show_st", "dev_sel", "get_ver", "bypass", "enable", "get_re", "get_f0"
};
	...
	struct device_attribute *attr, char *buf)
{
	...
	aw_cali_svc_cali_cmd(aw_dev, AW_CALI_CMD_RE, is_single_cali, CALI_OPS_HMUTE);
...
static ssize_t aw_cali_class_cali_re_show(struct  class *class, struct class_attribute *attr, char *buf)
{
	...
	aw_cali_svc_cali_cmd(local_dev, AW_CALI_CMD_RE, false, CALI_OPS_HMUTE);
	...
	switch (cmd) {
		...
		case AW_IOCTL_GET_RE: {		
			...
			calib_info = (struct calib_oeminfo *)data_ptr;
				if (is_single_cali) {
					aw_dev->cali_desc.cali_re = calib_info->calib_value[0];
					aw_dev_info(aw_dev->dev, "store re [%d]", aw_dev->cali_desc.cali_re);
				} else {
					ret = aw_dev_get_list_head(&dev_list);
					if (ret) {
						aw_dev_err(aw_dev->dev, "get dev list failed");
					...
					list_for_each (pos, dev_list) {
						local_dev = container_of(pos, struct aw_device, list_node);
						if (calib_info->calib_value[local_dev->channel] > 0) {
							aw_cali_store_cali_re(local_dev, calib_info->calib_value[local_dev->channel]);
						}
						aw_dev_info(aw_dev->dev, "store re [%d] for channel:%s", 
								calib_info->calib_value[local_dev->channel], ch_name[local_dev->channel]);
					}
				}
		} break;
		default:{
...
static ssize_t aw_cali_misc_read(struct file *filp, char __user *buf, size_t size, loff_t *pos)
{
	...
	case CALI_STR_CALI_RE_F0: {
		...
		data.pa_count = ret;
		if (ret <= 0) {
			aw_dev_err(aw_dev->dev, "get result of start_cali failed");
			return ret;
		} else {
			for (i = 0; i < data.pa_count; i++) {
				data.re[i] = temp_data[i];
	...
	if (copy_to_user((void __user *)buf, &data, sizeof(data))) {
...
static ssize_t aw_cali_misc_write(struct file *filp, const char __user *buf, size_t size, loff_t *pos)
{
	...
	int ret = 0;
	int enable = 1;
	struct list_head *dev_list, *pos1 = NULL;
	struct aw_device *local_dev;
	...
	case CALI_STR_BYPASS: {
		...
		list_for_each (pos1, dev_list) {
			local_dev = container_of(pos1, struct aw_device, list_node);
			local_dev->set_prof = AW_PROFILE_BYPASS;
			aw_dev_fw_update(local_dev, false);
		}
	...
	case CALI_STR_ENABLE: {
		...
		list_for_each (pos1, dev_list) {
			local_dev = container_of(pos1, struct aw_device, list_node);
			local_dev->set_prof = AW_PROFILE_MUSIC;
			aw_dev_fw_update(local_dev, false);
		}
...
static void aw_cali_parse_dt(struct aw_device *aw_dev)
{
	...
	if (ret < 0) {
		aw_dev_info(aw_dev->dev, " aw-cali-mode get failed ,user default attr way");
		desc->mode = AW_CALI_MODE_MISC;
		return;
	}

	if (!strcmp(cali_mode_str, "aw_class"))
...
void aw_cali_init_v2(struct aw_cali_desc *cali_desc)
{
...
void aw_cali_deinit_v2(struct aw_cali_desc *cali_desc)
{
	struct aw_device *aw_dev =
		container_of(cali_desc, struct aw_device, cali_desc);
	if (cali_desc->mode == AW_CALI_MODE_ATTR)
