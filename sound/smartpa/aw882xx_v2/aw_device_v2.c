		...
		struct aw_cfg_hdr *prof_hdr, struct aw_all_prof_info *all_prof_info)
{
	...
	for (i = 0; i < prof_hdr->a_ddt_num; i++) {
		if ((aw_dev->i2c->adapter->nr == cfg_dde[i].dev_bus) &&
			...
			if (ret < 0) {
				aw_dev_err(aw_dev->dev, "parse bus[0x%x] addr[0x%x] scene[%s] data failed",
					aw_dev->i2c->adapter->nr, aw_dev->i2c->addr,
					profile_name[cfg_dde[i].dev_profile]);
				return ret;
			}
			sec_num++;
		}
	...
	for (i = 0; i < prof_hdr->a_ddt_num; i++) {
		if ((aw_dev->index == cfg_dde[i].dev_index) &&
			...
			ret = aw_dev_parse_data_by_sec_type(aw_dev, prof_hdr, &cfg_dde[i],
					&all_prof_info->prof_desc[cfg_dde[i].dev_profile]);
			if (ret < 0) {
				aw_dev_err(aw_dev->dev, "parse index[%d] scene[%s] data failed",
					aw_dev->index, profile_name[cfg_dde[i].dev_profile]);
				return ret;
			}
			sec_num++;
		}
		...
		if ((aw_dev->index == cfg_dde[i].dev_index) &&
			...
			if (cfg_dde[i].data_type == ACF_SEC_TYPE_DSP) {
				ret = aw_dev_parse_raw_dsp(aw_dev,
					...
					&all_prof_info->prof_desc[cfg_dde[i].dev_profile]);
				if (ret < 0) {
					aw_dev_err(aw_dev->dev, "parse index[%d] scene[%s] skt data failed",
						aw_dev->index, profile_name[cfg_dde[i].dev_profile]);
					return ret;
				}
				sec_num++;
...
int aw_device_probe(struct aw_device *aw_dev)
{
	...
	aw_cali_init_v2(&aw_dev->cali_desc);
...
int aw_device_remove(struct aw_device *aw_dev)
{
	...
	aw_cali_deinit_v2(&aw_dev->cali_desc);
	/*aw_afe_deinit();*/
	return 0;
}
