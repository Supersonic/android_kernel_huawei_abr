...
int aw882xx_sysst_check(struct aw882xx *aw882xx)
{
	int ret = -1;
	unsigned char i;
	unsigned int reg_val = 0;
...
static void aw882xx_get_cfg_shift(struct aw882xx *aw882xx)
{
	...
	if (!chan_info->bin_cfg_name) {
		...
		if (!chan_info->name_suffix) {
			...
			snprintf(chan_info->bin_cfg_name[AW882XX_MODE_SPK_BYPASS_SHIFT],
				AW882XX_CFG_NAME_MAX, "%s.bin",
				chan_info->bin_cfg_name[AW882XX_MODE_SPK_BYPASS_SHIFT]);
		} else {
			snprintf(chan_info->bin_cfg_name[AW882XX_MODE_SPK_SHIFT],
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	if ((ucontrol->value.integer.value[0] < AW882XX_MODE_SHIFT_MAX)&&
		(ucontrol->value.integer.value[0] >= 0)){
			aw882xx->scene_mode = ucontrol->value.integer.value[0];
			aw882xx->init = AW882XX_INIT_ST;
	} else {
		aw_dev_err(aw882xx->dev,"%s: mode set value error, mode=%d\n", 
				__func__, ucontrol->value.integer.value[0]);
	}
