...
void aw882xx_get_re(struct aw_cali *cali){
	
	struct aw882xx *aw882xx = 
		container_of(cali, struct aw882xx, cali);
		
	aw_dev_err(aw882xx->dev, "%s:channel %d\n",
			__func__, aw882xx->chan_info.channel);
	if(aw882xx->chan_info.channel == AW882XX_CHANNLE_RIGHT) {
		aw882xx->cali.cali_re = MOHM_TO_FIXED_RE(aw882xx->cali.re[1]);
	} else {
		aw882xx->cali.cali_re = MOHM_TO_FIXED_RE(aw882xx->cali.re[0]);
	}
	aw_dev_err(aw882xx->dev, "%s:re value from oeminfo %d\n",
...
static int aw882xx_cali_re_startup(struct aw882xx *aw882xx)
{
	int ret;
...
static ssize_t aw882xx_calibration_read(struct file *file, char __user *buf,
											size_t count, loff_t *pos)
{
	struct aw882xx *aw882xx = NULL;
	struct aw_cali_data data;
	int i;
	int ret;
	...
	memset(&data, 0x00, sizeof(struct aw_cali_data));
	
	ret = aw882xx_sysst_check(aw882xx);
	if(ret == 0) {
		if(aw882xx->cali.get_re_f0 == 1) {
			if(aw882xx->cali.is_cali_started == 1) {
				aw882xx->cali.is_cali_started = 0;
			} else {
				aw_cali_get_re_from_dsp(aw882xx);
			}
		} else if (aw882xx->cali.get_re_f0 == 2 ) {
			aw_cali_get_f0_from_dsp(aw882xx);
		}
	}
	
	data.pa_count = aw882xx->cali.end_cali_num - aw882xx->cali.first_cali_num +1;
	for(i=0; i < data.pa_count; i++) {
		data.re[i] = aw882xx->cali.re[i];
		data.f0[i] = aw882xx->cali.f0[i];
	}
	ret = copy_to_user(buf, &data, sizeof(struct aw_cali_data));
	if(ret) {
		return -ret;
	} else {
		return count;
...
static ssize_t aw882xx_calibration_write(struct file *file, const char __user *buf,
											size_t count, loff_t *pos)
{
	struct aw882xx *aw882xx = NULL;
	char cali_buf[MAX_BUFFER_LENGTH] = {0};
	int ret;
	int bypass = 1;
	...
	memset(cali_buf, 0x00, MAX_BUFFER_LENGTH);
	if(count >= MAX_BUFFER_LENGTH) {
		ret = copy_from_user(cali_buf, buf, MAX_BUFFER_LENGTH);
	} else {
		ret = copy_from_user(cali_buf, buf, count);	
	}
	if(ret) {
		pr_err("copy_from_user returned error(%d)\n", ret);
		return ret;
		
	}
	
	pr_err("%s:copy_from_user returned str(%s)\n",__func__, cali_buf);
	if(strncmp(cali_buf, "start_cali", strlen("start_cali")) == 0) {
		ret = aw882xx_cali_re_startup(aw882xx);
		if(ret < 0) {
			aw_dev_err(aw882xx->dev, "%s: cali failed\n", __func__);
			memset(aw882xx->cali.re, 0, sizeof(aw882xx->cali.re));
		} else {
			aw882xx->cali.is_cali_started = 1;
			aw882xx->cali.get_re_f0 = 0;
		}
	} else if(strncmp(cali_buf, "get_re", strlen("get_re")) == 0) {
		aw882xx->cali.get_re_f0 = 1;
	} else if(strncmp(cali_buf, "get_f0", strlen("get_f0")) == 0) {
		aw882xx->cali.get_re_f0 = 2;
	} else if(strncmp(cali_buf, "bypass", strlen("bypass")) == 0) {
		bypass = 1;
		aw882xx->last_scene_mode = aw882xx->scene_mode;
		aw882xx->scene_mode = AW882XX_MODE_SPK_BYPASS_SHIFT;
		aw882xx->init = AW882XX_INIT_ST;
		ret = aw_send_module_enable(&bypass, AW_RX_MODULE);
		if(ret < 0) {
			aw_dev_err(aw882xx->dev, "%s: RX bypass failed: %d\n", __func__, ret);
		}
		ret = aw_send_module_enable(&bypass, AW_TX_MODULE);
		if(ret < 0) {
			aw_dev_err(aw882xx->dev, "%s: TX bypass failed: %d\n", __func__, ret);
		}		
		aw_dev_err(aw882xx->dev, "%s: TX&RX bypass success\n", __func__);
	} else if(strncmp(cali_buf, "enable", strlen("enable")) == 0) {
		bypass = 0;
		aw882xx->scene_mode = aw882xx->last_scene_mode;
		aw882xx->init  = AW882XX_INIT_ST;
		ret = aw_send_module_enable(&bypass, AW_RX_MODULE);
		if(ret < 0) {
			aw_dev_err(aw882xx->dev, "%s: RX enable failed: %d\n", __func__, ret);
		}
		ret = aw_send_module_enable(&bypass, AW_TX_MODULE);
		if(ret < 0) {
			aw_dev_err(aw882xx->dev, "%s: TX enable failed: %d\n", __func__, ret);
		}		
		aw_dev_err(aw882xx->dev, "%s: TX&RX enable success\n", __func__);	
	}
	return ret;
}
			...
			unsigned int cmd, unsigned long arg)
{
	...
	int32_t *p_data = NULL;
	struct calib_oeminfo *calib_info;
	...
	case AW882XX_IOCTL_GET_RE:
		...
		calib_info = (struct calib_oeminfo *)data_ptr;
		aw882xx->cali.re[0] = calib_info->calib_value[0];
		aw882xx->cali.re[1] = calib_info->calib_value[1];
		aw_dev_err(aw882xx->dev, "%s:Get Re value: %d\n",
					__func__, aw882xx->cali.re[0]);
		break;
	case AW882XX_IOCTL_SET_CALI_RE:
		if (copy_from_user(data_ptr,
			(void __user *)arg, data_len)) {
...
static const struct file_operations aw882xx_fops = {
	...
	.read = aw882xx_calibration_read,
	.write = aw882xx_calibration_write,
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (aw882xx->cali.end_cali_num - aw882xx->cali.first_cali_num) {
		aw_dev_err(aw882xx->dev, "%s:Re = left:%d mOhms right:%d mOhms\n", __func__,
				aw882xx->cali.re[0], aw882xx->cali.re[1]);
		aw_dev_err(aw882xx->dev, "%s:F0 = left:%d right:%d\n", __func__,
				aw882xx->cali.f0[0], aw882xx->cali.f0[1]);
		len += snprintf(buf+len,
			PAGE_SIZE - len, "Re-L:%d mOhms Re-R:%d mOhms\n",
			aw882xx->cali.re[0], aw882xx->cali.re[1]);
		len += snprintf(buf+len,
			PAGE_SIZE - len, "F0-L:%d F0-R:%d\n",
			aw882xx->cali.f0[0], aw882xx->cali.f0[1]);
	} else {
		aw_dev_err(aw882xx->dev, "%s:Re = %d mOhms, F0 = %d\n",__func__,
				aw882xx->cali.re[aw882xx->chan_info.channel],
				aw882xx->cali.f0[aw882xx->chan_info.channel]);
		len += snprintf(buf+len,
			PAGE_SIZE - len, "Re = %d mOhms\n",
			aw882xx->cali.re[aw882xx->chan_info.channel]);
		len += snprintf(buf+len,
			PAGE_SIZE - len, "F0 = %d\n",
			aw882xx->cali.f0[aw882xx->chan_info.channel]);
