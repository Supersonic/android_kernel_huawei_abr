...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,4,0))
static int __do_vip_balance_kernel_5_4(void *data)
{
	...
	if (cpumask_test_cpu(src_cpu, &cluster))
		cpumask_and(cpus_ptr, &cluster, p->cpus_ptr);
	else
		cpumask_and(cpus_ptr,
			&slow_cpu_mask, p->cpus_ptr);
	raw_spin_unlock(&src_rq->lock);
...
#else
static int __do_vip_balance_kernel_less_5_4(void *data)
{
	struct rq *src_rq = data;
	struct rq *dst_rq = NULL;
	int src_cpu = cpu_of(src_rq);
...
{
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,4,0))
	return __do_vip_balance_kernel_5_4(data);
#else
	return __do_vip_balance_kernel_less_5_4(data);
#endif
}
