...
static struct hwqi_handle *g_idtp9415_handle;
...
static int idtp9415_read_byte(u16 reg, u8 *data)
{
	...
	return idtp9415_read_block(di, reg, data, POWER_BYTE_LEN);
}
...
static int idtp9415_read_word(u16 reg, u16 *data)
{
	...
	ret = idtp9415_read_block(di, reg, buff, POWER_WORD_LEN);
	if (ret)
		return -EIO;
...
static int idtp9415_write_byte(u16 reg, u8 data)
{
	...
	return idtp9415_write_block(di, reg, &data, POWER_BYTE_LEN);
}
...
static int idtp9415_write_word(u16 reg, u16 data)
{
	...
	return idtp9415_write_block(di, reg, buff, POWER_WORD_LEN);
}
...
static int idtp9415_get_chip_id(u16 *chip_id)
{
	...
	*chip_id = chip_id_l | (chip_id_h << POWER_BITS_PER_BYTE);
...
static int idtp9415_get_fw_version(u8 *data, int len, void *dev_data)
{
	...
	data[0] = (u8)((chip_info.major_fw_ver >> 0) & POWER_MASK_BYTE);
	data[1] = (u8)((chip_info.major_fw_ver >> POWER_BITS_PER_BYTE) & POWER_MASK_BYTE);
	data[2] = (u8)((chip_info.minor_fw_ver >> 0) & POWER_MASK_BYTE);
	data[3] = (u8)((chip_info.minor_fw_ver >> POWER_BITS_PER_BYTE) & POWER_MASK_BYTE);
...
static void idtp9415_get_rx_iavg(int *rx_iavg, void *dev_data)
{
	struct idtp9415_dev_info *di = dev_data;
...
static void idtp9415_get_rx_imax(int *rx_imax, void *dev_data)
{
	struct idtp9415_dev_info *di = dev_data;
...
static int idtp9415_get_rx_fod(char *coef_str, int len, void *dev_data)
{
	...
	for (i = 0; i < IDTP9415_RX_FOD_LEN; i++) {
		ret = idtp9415_read_word(IDTP9415_RX_FOD_ADDR + i * POWER_WORD_LEN,
			&fod_coef[i]);
		if (ret) {
...
static int idtp9415_set_rx_fod(const char *fod_str, void *dev_data)
{
	...
	for (i = 0; i < IDTP9415_RX_FOD_LEN; i++) {
		...
		ret = idtp9415_write_word(IDTP9415_RX_FOD_ADDR + i * POWER_WORD_LEN,
			(u16)val);
		if (ret) {
...
static int idtp9415_init_rx_fod_coef(struct idtp9415_dev_info *di, int tx_type)
{
	...
	for (i = 0; i < IDTP9415_RX_FOD_LEN; i++) {
		addr = IDTP9415_RX_FOD_ADDR + i * POWER_WORD_LEN;
...
static int idtp9415_data_received_handler(struct idtp9415_dev_info *di)
{
	...
	case HWQI_CMD_ACK_BST_ERR:
		...
		if (g_idtp9415_handle &&
			...
			g_idtp9415_handle->hdl_non_qi_fsk_pkt(buff, HWQI_PKT_LEN, di);
		break;
	default:
		break;
	}
...
static void idtp9415_irq_work(struct work_struct *work)
{
	...
	if (!di) {
		power_wakeup_unlock(g_idtp9415_wakelock, false);
		return;
	}
...
static irqreturn_t idtp9415_interrupt(int irq, void *_di)
{
	...
	power_wakeup_lock(g_idtp9415_wakelock, false);
	hwlog_info("[%s] ++\n", __func__);
	if (di->irq_active) {
		disable_irq_nosync(di->irq_int);
		di->irq_active = false;
...
static struct hwqi_ops idtp9415_qi_protocol_ops = {
	.chip_name              = "idtp9415",
...
static int idtp9415_ops_register(struct idtp9415_dev_info *di)
{
	...
	ret = wireless_fw_ops_register(&g_idtp9415_fw_ops, di->ic_type);
	if (ret) {
	...
	ret = hwqi_ops_register(&idtp9415_qi_protocol_ops, di->ic_type);
	if (ret) {
		hwlog_err("ops_register: register qi_ops failed\n");
		return ret;
	}
	g_idtp9415_handle = hwqi_get_handle();
...
static void idtp9415_shutdown(struct i2c_client *client)
{
	...
	if (wlrx_get_wireless_channel_state() == WIRELESS_CHANNEL_ON) {
		...
		(void)idtp9415_set_vfc(ADAPTER_5V * POWER_MV_PER_V, di);
		(void)idtp9415_set_rx_vout(ADAPTER_5V * POWER_MV_PER_V, di);
	...
	const struct i2c_device_id *id)
{
	...
	g_idtp9415_wakelock = power_wakeup_source_register(di->dev,
		"idtp9415_wakelock");
	...
	wlic_iout_init(di->ic_type, np, &di->rx_imax);
	if (idtp9415_is_tx_exist(di)) {
		idtp9415_rx_clear_interrupt(di, IDTP9415_ALL_INTR_CLR);
...
ops_regist_fail:
	power_wakeup_source_unregister(g_idtp9415_wakelock);
