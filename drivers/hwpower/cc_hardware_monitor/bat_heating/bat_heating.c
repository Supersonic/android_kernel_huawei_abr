...
static int bat_heating_drm_notifier_call(struct notifier_block *nb,
	unsigned long event, void *data)
{
	struct bat_heating_dev *l_dev = bat_heating_get_dev();
	...
	struct bat_heating_dev *l_dev)
{
	(void)power_dts_read_str2int(power_dts_tag(HWLOG_TAG), np,
		"panel_client", &l_dev->panel_client,
		BAT_HEATING_PANEL_CLIENT_FB);
	(void)power_dts_read_str2int(power_dts_tag(HWLOG_TAG), np,
...
{
#ifdef CONFIG_FB
	if (l_dev->panel_client == BAT_HEATING_PANEL_CLIENT_FB) {
		l_dev->fb_nb.notifier_call = bat_heating_fb_notifier_call;
		(void)fb_register_client(&l_dev->fb_nb);
	}
#endif

#ifdef CONFIG_DRM_PANEL
	if (l_dev->panel_client == BAT_HEATING_PANEL_CLIENT_DRM) {
		l_dev->fb_nb.notifier_call = bat_heating_drm_notifier_call;
		(void)lcd_kit_drm_notifier_register(0, &l_dev->fb_nb);
	}
#endif
}
...
#ifdef CONFIG_FB
	if (l_dev->panel_client == BAT_HEATING_PANEL_CLIENT_FB)
		(void)fb_unregister_client(&l_dev->fb_nb);
#endif
