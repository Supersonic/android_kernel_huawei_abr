								...
								uint8_t *ram_data, uint32_t len)
{
	...
	if ((cont_data == NULL) || (ram_data == NULL)) {
		aw_err("cont_data or ram_data is null");
		return -EINVAL;
	}
	for (i = 0; i < len; i++) {
								...
								struct aw_haptic_container *awinic_cont)
{
	...
	if ((aw_haptic == NULL) || (awinic_cont == NULL)) {
		aw_err("aw_haptic or awinic_cont is null");
		return -EINVAL;
	}
				...
				struct device_attribute *attr, char *buf)
{
	...
	if (!cdev) {
		aw_err ("cdev error is %d", PTR_ERR(cdev));
		return PTR_ERR(cdev);
	}
