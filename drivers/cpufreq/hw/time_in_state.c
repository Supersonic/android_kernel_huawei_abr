...
ssize_t hw_time_in_state_show(int cpu, char *buf)
{
...
static int hw_time_adj_freq_get(int cpu, u64 *timeadjfreq)
{
	unsigned int i;
...
int perf_ctrl_get_cpu_busy_time(void __user *uarg)
{
	...
	for (i = 0; i < busy_time.cpu_count; i++) {
		ret = hw_time_adj_freq_get(i, &busy_time.time_adj_freq[i]);
		if (ret < 0)
			return ret;
	}
...
int hw_time_in_freq_get(int cpu, u64 *freqs, int freqs_len)
{
	unsigned int i, j, pos;
	struct time_in_state *icpu = NULL;
