...
static int switch_show(struct seq_file *m, void *unused)
{
...
{
	return single_open(file, switch_show, inode->i_private);
}
...
retry:
	...
	while (1) {
		...
		binder_inner_proc_lock(proc);
#if defined(CONFIG_HW_BINDER_SCHED) || defined(CONFIG_HW_BINDER_FG_REQ_FIRST)
					     ...
					     struct binder_transaction *t)
{
	...
	to_proc = t->to_proc;
	seq_printf(m,
		   "%s %d: %pK from %d:%d to %d:%d code %x flags %x pri %d:%d r%d wait:%lld.%lld s",
		   prefix, t->debug_id, t,
		   ...
		   now > t->timestamp ? (now - t->timestamp) / 1000000000 : 0,
		   now > t->timestamp ? (now - t->timestamp) % 1000000000 : 0);
	spin_unlock(&t->lock);
