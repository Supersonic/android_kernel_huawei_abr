...
uint32_t fi_ipv4_flow_hash(struct fi_pkt_parse *pktinfo, int str_len, bool exchange)
{
	int ret;
	static u32 fi_ipv4_flow_secret __read_mostly;
	__be32 addr1 = pktinfo->msg.flow_msg.ipv4_sip;
	__u16 port1 = pktinfo->msg.flow_msg.sport;
	__be32 addr2 = pktinfo->msg.flow_msg.ipv4_dip;
	__be16 port2 = pktinfo->msg.flow_msg.dport;
	char *flow = pktinfo->msg.flow_msg.flow;
...
{
	__be32 saddr = pktinfo->msg.flow_msg.ipv4_sip;
	__u16 sport = pktinfo->msg.flow_msg.sport;
	__be32 daddr = pktinfo->msg.flow_msg.ipv4_dip;
	__be16 dport = pktinfo->msg.flow_msg.dport;
	fi_flow_cb flow_cb = pktinfo->flow_cb;
			...
			struct fi_pkt_parse *pktinfo, bool addflow, struct sock *sk)
{
	...
	__be32 daddr = pktinfo->msg.flow_msg.ipv4_dip;
	__be16 dport = pktinfo->msg.flow_msg.dport;
	bool *newflow = &(pktinfo->msg.flow_msg.new_flow);
	...
	if (head != NULL && newflow != NULL) {
		...
		if (addflow) {
			...
			*newflow = true;
			return fi_ipv4_flow_add(head, uid, pktinfo, sk);
		}
		*newflow = false;
	}
	return NULL;
...
uint32_t fi_ipv6_flow_hash(struct fi_pkt_parse *pktinfo, int str_len, bool exchange)
{
	int ret;
	...
	u32 hash1;
	u32 hash2;
	struct in6_addr *addr1 = (struct in6_addr *)&(pktinfo->msg.flow_msg.ipv6_sip);
	__u16 port1 = pktinfo->msg.flow_msg.sport;
	struct in6_addr *addr2 = (struct in6_addr *)&(pktinfo->msg.flow_msg.ipv6_dip);
	__be16 port2 = pktinfo->msg.flow_msg.dport;
	char *flow = pktinfo->msg.flow_msg.flow;
	...
	struct fi_pkt_parse *pktinfo, struct sock *sk)
{
	struct fi_flow_node *newnode = NULL;
	int ret;
	struct in6_addr *saddr = (struct in6_addr *)&(pktinfo->msg.flow_msg.ipv6_sip);
	__u16 sport = pktinfo->msg.flow_msg.sport;
	struct in6_addr *daddr = (struct in6_addr *)&(pktinfo->msg.flow_msg.ipv6_dip);
	__be16 dport = pktinfo->msg.flow_msg.dport;
	fi_flow_cb flow_cb = pktinfo->flow_cb;
	...
	struct fi_pkt_parse *pktinfo, bool addflow, struct sock *sk)
{
	...
	if (saddr != NULL && daddr != NULL && head != NULL && newflow != NULL) {
		...
		if (addflow) {
			...
			*newflow = true;
			return fi_ipv6_flow_add(head, uid, pktinfo, sk);
		}
		*newflow = false;
	}
	return NULL;
