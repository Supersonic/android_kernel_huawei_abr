...
static int mmc_lock_sd_init_card(struct mmc_card *card, bool binit)
{
	...
	int err;
	unsigned long clk_scaling_highest_backup = 0;
	...
	host = card->host;
	clk_scaling_highest_backup = mmc_sd_get_max_clock(card);
	...
	/*
	 * When the locked sd card is inserted into the phone, the card->clk_scaling_highest value is not signed.
	 * So we need to re-match the devfreq after unlocking the sdcard to make sure that the sdcard works with the corrert freq.
	 */
	card->clk_scaling_highest = mmc_sd_get_max_clock(card);
	card->clk_scaling_lowest = host->f_min;
	if(card->clk_scaling_highest > clk_scaling_highest_backup) {
		...
		if (host->clk_scaling.enable) {
			...
			if(err) {
				printk("mmc_exit_clk_scaling failed when sd_reinit\n");
				return err;
			}
		}
		err = mmc_init_clk_scaling(host);
		if(err) {
			printk("mmc_init_clk_scaling failed when sd_reinit\n");
			return err;
		}
	}
...
static ssize_t mmc_lockable_store(struct device *dev, struct device_attribute *att,
