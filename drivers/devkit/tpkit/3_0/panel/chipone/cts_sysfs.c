		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	TS_LOG_INFO("Write firmware register '%.*s'", (int)count, buf);

	if (argc < 2) {
		TS_LOG_ERR("Too few args %d", argc);
		return -EFAULT;
	}

	ret = kstrtou16(argv[0], 0, &addr);
	if (ret) {
		TS_LOG_ERR("Invalid address %s", argv[0]);
		return -EINVAL;
	}

	data = (u8 *)kmalloc(argc - 1, GFP_KERNEL);
	if (data == NULL) {
		TS_LOG_ERR("Allocate buffer for write data failed\n");
		return -ENOMEM;
	}
	...
	for (i = 1; i < argc; i++) {
		...
		if (ret) {
			TS_LOG_ERR("Invalid value %s", argv[i]);
			goto free_data;
		}
	}
	...
	if (ret) {
		TS_LOG_ERR("Write firmware register addr: 0x%04x size: %d failed",
			addr, argc - 1);
		goto free_data;
	}
...
#define PRINT_ROW_SIZE (16)
	...
	TS_LOG_INFO("Read firmware register ");
	...
	TS_LOG_INFO("Read firmware register from 0x%04x size %u", addr, size);
...
#define PRINT_ROW_SIZE (16)
	...
	TS_LOG_INFO("Read hw register from 0x%08x size %u", addr, size);
	cts_lock_device(cts_dev);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	TS_LOG_INFO("Write hw register from 0x%08x size %u", addr, argc - 1);
	...
	if (ret) {
		TS_LOG_ERR("Write hw register failed %d", ret);
	}
...
static DEVICE_ATTR(curr_ddi_version, S_IRUSR | S_IRGRP,
	curr_ddi_version_show, NULL);
...
static DEVICE_ATTR(esd_protection, S_IRUSR | S_IRGRP,
	esd_protection_show, NULL);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	TS_LOG_INFO("Write firmware monitor mode to '%c', %s",
	...
	cts_unlock_device(cts_dev);
	if (ret) {
		TS_LOG_ERR("Write firmware monitor enable register failed %d", ret);
		return -EIO;
	}

	if ((value & BIT(0)) && enable) {
		TS_LOG_INFO("Monitor mode already enabled");
	} else if ((value & BIT(0)) == 0 && enable == 0) {
		TS_LOG_INFO("Monitor mode already disabled");
	} else {
		if (enable) {
			value |= BIT(0);
		} else {
...
static DEVICE_ATTR(monitor_mode, S_IRUSR | S_IRGRP,
...
static DEVICE_ATTR(auto_compensate, S_IRUSR | S_IRGRP,
	auto_compensate_show, NULL);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (argc > 1) {
		if (strncasecmp(argv[1], "flash", 5) == 0) {
			...
			TS_LOG_ERR("Invalid location '%s', must be 'flash' or 'sram'", argv[1]);
			return -EINVAL;
		}
	}

	TS_LOG_INFO("Update driver builtin firmware '%s' to %s",
	...
	if (firmware) {
		...
		TS_LOG_ERR("Firmware '%s' NOT found", argv[0]);
		return -ENOENT;
	}
...
static DEVICE_ATTR(driver_builtin_firmware, S_IWUSR | S_IRUSR | S_IRGRP,
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	TS_LOG_INFO("Update firmware from file '%s'", argv[0]);

	firmware = cts_request_firmware_from_fs(cts_dev, argv[0]);
	if (firmware == NULL) {
		TS_LOG_ERR("Request firmware from file '%s' failed", argv[0]);
		return -ENOENT;
	}
...
static DEVICE_ATTR(info, S_IRUSR | S_IRGRP, flash_info_show, NULL);
		...
		struct device_attribute *attr, char *buf)
{
	...
	TS_LOG_INFO("Read flash from 0x%06x size %u%s%s",
	...
	if (argc == 3) {
		...
		TS_LOG_INFO("Write flash data to file '%s'", argv[2]);
...
err_post_flash_operation:
	...
	if (enabled) {
		...
		if (r) {
			cts_unlock_device(cts_dev);
			count += scnprintf(buf, PAGE_SIZE, "Start device failed %d", r);
		}
	}
err_free_data:
	cts_unlock_device(cts_dev);
...
static DEVICE_ATTR(read, S_IWUSR | S_IRUSR | S_IRGRP,
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (argc != 2) {
		TS_LOG_ERR("Invalid num args %d", argc);
		return -EFAULT;
	}

	ret = kstrtou32(argv[0], 0, &flash_addr);
	if (ret) {
		TS_LOG_ERR("Invalid flash addr: %s", argv[0]);
		return -EINVAL;
	}
	ret = kstrtou32(argv[1], 0, &size);
	if (ret) {
		TS_LOG_ERR("Invalid size: %s", argv[1]);
		return -EINVAL;
	}

	TS_LOG_INFO("Erase flash from 0x%06x size %u", flash_addr, size);
	...
	ret = cts_prepare_flash_operation(cts_dev);
	if (ret) {
		TS_LOG_ERR("Prepare flash operation failed %d", ret);
		cts_unlock_device(cts_dev);
		return ret;
	}
	...
	if (ret) {
		TS_LOG_ERR("Erase flash from 0x%06x size %u failed %d",
			flash_addr, size, ret);
		goto err_post_flash_operation;
	}
...
err_post_flash_operation:
	...
	if (!program_mode) {
		int r = cts_enter_normal_mode(cts_dev);
		if (r) {
			TS_LOG_ERR("Enter normal mode failed %d", r);
		}
	}
		...
		struct device_attribute *attr, char *buf)
{
	...
	TS_LOG_INFO("Open test, threshold = %u", min);
		...
		struct device_attribute *attr, char *buf)
{
	...
	TS_LOG_INFO("Short test, threshold = %u", min);
		...
		struct device_attribute *attr, char *buf)
{
	...
	TS_LOG_INFO("Compensate cap test, min: %u, max: %u",
		 min, max);
...
static DEVICE_ATTR(compensate_cap_test, S_IWUSR | S_IRUSR | S_IRGRP,
		...
		struct device_attribute *attr, char *buf)
{
	...
	TS_LOG_INFO("Rawdata test, frames: %u min: %d, max: %d",
		...
		struct device_attribute *attr, char *buf)
{
	...
	TS_LOG_INFO("Noise test, frames: %u threshold: %d",
...
#define COMP_CAP_TEST_DATA_FILENAME "comp-cap-test-data.txt"
	...
	TS_LOG_INFO("Factory test: rawdata: [%u, %u] noise: %u, "
...
static DEVICE_ATTR(factory_test, S_IRUSR | S_IRGRP,
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (*argv[0] == '1' || tolower(*argv[0]) == 'y') {
		...
		if (ret) {
			TS_LOG_ERR("Exit program mode failed %d", ret);
			return ret;
		}
	} else {
		TS_LOG_ERR("Invalid args");
	}
		...
		struct device_attribute *attr, char *buf)
{
	...
	TS_LOG_INFO("Show diffdata");

	diffdata = (s16 *)kmalloc(DIFFDATA_BUFFER_SIZE(cts_dev), GFP_KERNEL);
	if (diffdata == NULL) {
		TS_LOG_ERR("Allocate memory for diffdata failed");
		return -ENOMEM;
	}
	...
	ret = cts_enable_get_rawdata(cts_dev);
	if (ret) {
		TS_LOG_ERR("Enable read diff data failed %d", ret);
	...
	if(ret) {
		TS_LOG_ERR("Get diff data failed %d", ret);
		data_valid = false;
		// Fall through to disable get diffdata
	}
	ret = cts_disable_get_rawdata(cts_dev);
	if (ret) {
		TS_LOG_ERR("Disable read diff data failed %d", ret);
...
static DEVICE_ATTR(baseline, S_IRUSR | S_IRGRP, baseline_show, NULL);
		...
		struct device_attribute *attr, char *buf)
{
	...
	ret = cts_enable_get_rawdata(cts_dev);
	if (ret) {
		TS_LOG_ERR("Enable read raw data failed %d", ret);
		goto err_free_diffdata;
	}
	...
	TS_LOG_INFO("frame %d, file:%s", frame, argv[1]);
	for (i = 0; i < frame; i++) {
		ret = cts_get_rawdata(cts_dev, rawdata);
		if(ret) {
			TS_LOG_ERR("Get raw data failed %d", ret);
			data_valid = false;
		}
		else {
			data_valid = true;
		...
		if (data_valid) {
			...
			//TS_LOG_INFO("manualdiffdata_show:%d, %d", i, frame);
			if (argc == 2) {
				pos = file->f_pos;
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,14,0)
...
#endif
				if (ret != count) {
					TS_LOG_ERR("Write data to file '%s' failed %d", argv[1], ret);
				}
				file->f_pos += count;
				count = 0;
			}
	...
	ret = cts_disable_get_rawdata(cts_dev);
	if (ret) {
		TS_LOG_ERR("Disable read raw data failed %d", ret);
		// Fall through to show diffdata
	}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (strncasecmp("updatebase", argv[0], 10) == 0) {
		...
		//TS_LOG_INFO("update base successful");
		ret = cts_disable_get_rawdata(cts_dev);
		if (ret) {
			TS_LOG_ERR("Disable read raw data failed %d", ret);
		}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	TS_LOG_INFO("jitter test frame: %d", jitter_test_frame);
	return count;
}
		...
		struct device_attribute *attr, char *buf)
{
	...
	TS_LOG_INFO("Read '%s'", attr->attr.name);
...
static DEVICE_ATTR(compensate_cap, S_IRUSR | S_IRGRP,
	compensate_cap_show, NULL);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	TS_LOG_INFO("Write RESET-PIN");
	TS_LOG_INFO("Chip status maybe changed");
	...
	struct device_attribute *attr, char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "spi_speed=%d\n",
		((struct chipone_ts_data *)dev_get_drvdata(dev))->spi_max_speed);
}
		...
		const char *buf, size_t count)
{
	...
	ret = kstrtou32(argv[0], 0, &s);
	if (ret) {
		TS_LOG_ERR("Invalid spi speed: %s", argv[0]);
		return -EINVAL;
	}

	((struct chipone_ts_data *)dev_get_drvdata(dev))->spi_max_speed = s;
...
int cts_sysfs_add_device(struct device *dev)
{
	...
	TS_LOG_INFO("Add device attr groups");
	...
	if (ret) {
		TS_LOG_ERR("Add device attr failed %d", ret);
		return ret;
	}

	manualdiff_base = (s16 *)kzalloc(DIFFDATA_BUFFER_SIZE(cts_dev), GFP_KERNEL);
	if (manualdiff_base == NULL) {
		TS_LOG_ERR("Malloc manualdiff_base failed");
		return -ENOMEM;
	}

	ret = sysfs_create_link(NULL, &dev->kobj, "chipone-tddi");
	if (ret) {
		TS_LOG_ERR("Create sysfs link error:%d", ret);
	}
	return 0;
}
...
void cts_sysfs_remove_device(struct device *dev)
{
	...
	TS_LOG_INFO("Remove device attr groups");
