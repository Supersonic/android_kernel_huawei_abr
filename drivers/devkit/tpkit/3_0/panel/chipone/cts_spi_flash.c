		...
		u32 sector_addr, int retry)
{
	...
	TS_LOG_INFO("  Erase sector 0x%06x", sector_addr);
...
int cts_prepare_flash_operation(struct cts_device *cts_dev)
{
	...
	if (ret) {
		TS_LOG_ERR("Read DIVA failed %d", ret);
	} else {
		TS_LOG_DEBUG("Device DIVA = %d", diva);
	}

	if (ret == 0 && diva == 0x0C) {
		TS_LOG_INFO("DIVA is ready already");
	} else {
		int retries;
		...
		if (ret) {
			TS_LOG_ERR("Write DIVA failed %d", ret);
		...
		if (ret) {
			TS_LOG_ERR("Reset sfctl failed %d", ret);
			goto err_enter_normal_mode;
		}
		...
		TS_LOG_ERR("Wait SFCTRL ready failed %d", ret);
		// Go through and try
	}
...
init_flash:
	if (cts_dev->flash == NULL) {
		...
		if ((ret = probe_flash(cts_dev)) != 0) {
			...
			TS_LOG_ERR("Probe flash failed %d", ret);
			return 0;
		}
	}
...
{
	TS_LOG_INFO("Post flash operation");
		...
		u32 flash_addr, u32 sram_addr, size_t size, int retry)
{
	...
	TS_LOG_INFO("Read to sram actually from 0x%06x size %zu", flash_addr, size);
	...
	do {
		...
		if(ret) {
			TS_LOG_ERR("Read from 0x%06x to sram 0x%06x size %zu failed %d retries %d",
		...
		u32 flash_addr, u32 sram_addr, size_t size, u32 crc, int retry)
{
	...
	do {
		...
		if (ret) {
			TS_LOG_ERR("Get crc for read from 0x%06x to sram 0x%06x size %zu "
					"failed %d retries %d",
				flash_addr, sram_addr, size, ret, retries);
			continue;
		}
		...
		TS_LOG_ERR("Check crc for read from 0x%06x to sram 0x%06x size %zu "
		...
		u32 flash_addr, const void *src, size_t size)
{
	...
	while (size) {
		...
		if(ret) {
			TS_LOG_ERR("Program to 0x%06x size %zu failed %d", flash_addr, l, ret);
			return ret;
		}
...
int cts_erase_flash(const struct cts_device *cts_dev, u32 addr, size_t size)
{
	...
	if (flash == NULL || sfctrl == NULL ||
		sfctrl->ops == NULL || sfctrl->ops->se == NULL ||
		sfctrl->ops->be == NULL) {
		TS_LOG_ERR("Oops");
		return -EINVAL;
	}
	...
	if (flash->block_size) {
		...
		while (size >= flash->block_size) {
			...
			if (ret) {
				TS_LOG_ERR("Erase block 0x%06x size 0x%04zx failed %d",
