...
#define RDSR_XCHG_SRAM_ADDR (cts_dev->hwdata->sram_size - 1)
	...
	if (ret) {
		TS_LOG_ERR("Read exchange sram failed %d", ret);
		return ret;
	}
...
#define FLASH_SR_WIP BIT(0) /*!< Write in progress */
	...
	do {
		...
		if (ret) {
			TS_LOG_ERR("Read flash status register failed %d", ret);
			return ret;
		}
		...
		u8 cmd, void *data, u32 flash_addr, u32 sram_addr, size_t size, u32 flags)
{
	...
	if (flags & SFCTRLv2_OPFLAG_SRAM_DATA_XCHG) {
		...
		if ((!(flags & SFCTRLv2_OPFLAG_READ)) && data) {
			...
			if (ret) {
				TS_LOG_ERR("Write data to exchange sram 0x%06x size %zu failed %d",
					sram_addr, size, ret);
				return ret;
			}
		}
	...
	if (flags & SFCTRLv2_OPFLAG_WAIT_WIP_CLR &&
		...
		TS_LOG_ERR("Wait WIP clear failed %d", ret);
		return ret;
	}
	...
	if ((flags & SFCTRLv2_OPFLAG_READ) && data) {
		...
		if (ret) {
			TS_LOG_ERR("Read data from exchange sram 0x%06x size %zu failed %d",
				sram_addr, size, ret);
			return ret;
		}
	}
