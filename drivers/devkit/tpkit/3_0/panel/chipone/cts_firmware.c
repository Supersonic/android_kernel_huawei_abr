...
u32 cts_data_check(const u8 *data, size_t len)
{
		...
		const struct cts_firmware *firmware)
{
	...
	crc = cts_data_check(FIRMWARE_SECTION(firmware), FIRMWARE_SECTION_SIZE(firmware));
	if (crc != FIRMWARE_SECTION_CRC(firmware)) {
		TS_LOG_ERR("Firmware-section crc mismatch crc-section %08x != %08x"
				", File maybe broken!!!",
	...
	if (DDIPARAM_SECTION_ENABLE(firmware) == CTS_SECTION_ENABLE_FLAG) {
		crc = cts_data_check(DDIPARAM_SECTION(firmware), DDIPARAM_SECTION_SIZE(firmware));
		if (crc != DDIPARAM_SECTION_CRC(firmware)) {
			TS_LOG_ERR("DDIParam-section crc mismatch crc-section %08x != %08x"
					", File maybe broken!!!",
				crc, DDIPARAM_SECTION_CRC(firmware));
			return false;
		}
	} else {
		TS_LOG_INFO("DDIParam-section is NOT enabled");
	}
		...
		struct cts_firmware_sect_info *info)
{
	...
	info->firmware_sect_crc = cts_data_check(firmware->data, firmware->size);
		...
		struct cts_firmware_sect_info *info)
{
	...
	TS_LOG_INFO("  Firmware section size: %zu", info->firmware_sect_size);
	if (info->ddiparam_crc_sect) {
		TS_LOG_INFO("  DDIParam section size: %zu", info->ddiparam_sect_size);
	}
...
const struct cts_firmware *cts_request_driver_builtin_firmware_by_name(const char *name)
{
	...
	TS_LOG_INFO("Request driver builtin by name '%s'", name);
	...
	for (i = 0; i < NUM_DRIVER_BUILTIN_FIRMWARE; i++, firmware++) {
		if (strcmp(firmware->name, name) == 0) {
			if (is_firmware_valid(firmware)) {
				TS_LOG_INFO("Found driver builtin '%s' "
			...
			TS_LOG_ERR("Found driver builtin '%s' "
...
const struct cts_firmware *cts_request_driver_builtin_firmware_by_index(u32 index)
{
	...
	TS_LOG_INFO("Request driver builtin by index %u", index);
	...
	if (index < NUM_DRIVER_BUILTIN_FIRMWARE) {
		...
		TS_LOG_ERR("Request driver builtin by index %u too large >= %zu",
	...
	const char *filepath, int curr_version)
{
	...
	if (IS_ERR(file)) {
		TS_LOG_ERR("Open file '%s' failed %ld", filepath, PTR_ERR(file));
		return PTR_ERR(file);
	}

	firmware->size = file_inode(file)->i_size;
	if (!is_firmware_size_valid(firmware)) {
		TS_LOG_INFO("File '%s' size: %zu invalid", filepath, firmware->size);
		ret = -EINVAL;
		goto err_close_file;
	}
...
#endif
	if (read_size < 0) {
		TS_LOG_ERR("Read version from offset 0x100 failed");
		ret = -EIO;
		goto err_close_file;
	}
	...
	if (version <= curr_version) {
		TS_LOG_INFO("File '%s' size: %zu version: %04x <= %04x",
			filepath, firmware->size, version, curr_version);
		ret = -EINVAL;
		goto err_close_file;
	}

	TS_LOG_INFO("File '%s' size: %zu version: %04x",
		filepath, firmware->size, version);

	firmware->data = (u8 *) vmalloc(firmware->size);
	if (firmware->data == NULL) {
		TS_LOG_ERR("Request form fs alloc firmware data failed");
		ret = -ENOMEM;
		goto err_close_file;
	}
	...
	if (read_size < 0 || read_size != firmware->size) {
		TS_LOG_ERR("Request from fs read whole file failed %d", read_size);
		ret = -EIO;
		goto err_free_firmware_data;
	}

	r_close = filp_close(file, NULL);
	if (r_close)
		TS_LOG_ERR("Close file '%s' failed %d", filepath, r_close);
	...
	const char *name, struct device *device, int curr_version)
{
	...
	if (curr_version > 0 && curr_version >= FIRMWARE_VERSION(firmware)) {
		TS_LOG_INFO("Request firmware file with version 0x%04x < 0x%04x",
	...
	const char *filepath, u16 curr_version)
{
	...
	if (filepath[0] == '/') { /* Full path */
		TS_LOG_INFO("Filepath is fullpath, direct read it out");
		ret = cts_request_firmware_full_filepath(firmware,
			filepath, curr_version);
	} else { /* ONLY file name is givin */
		TS_LOG_INFO("Filepath is only filename, use request_firmware()");
		...
		const char *filepath, bool to_flash)
{
	...
	if (ret) {
		TS_LOG_ERR("Update boot firmware failed %d\n", ret);
		goto err_release_firmware;
	}
		 ...
		 u8 *buf, bool calc_crc, u32 crc)
{
	...
	if (buf == NULL) {
		...
		if (buf == NULL) {
			TS_LOG_ERR("Validate flash data allocate mem failed");
			return -ENOMEM;
		}
	...
	if (ret) {
		TS_LOG_ERR("Read flash from 0x%06x size %zu failed %d",
			flash_addr, size, ret);
		goto err_free_buf;
	}
	...
	for (i = 0; i < size; i++) {
		if (buf[i] != data[i]) {
			if (ret == 0) {
				TS_LOG_ERR("Flash data from 0x%06x size %zu first bytes diff:\n",
					flash_addr, size);
			}

			if (ret < 100) {
				TS_LOG_ERR("  0x%06x: %02x %02x", i, buf[i], data[i]);
			} else if (ret == 100) {
				TS_LOG_ERR("  ...");
			}
			ret++;
		...
		const struct cts_firmware_sect_info *firmware_info)
{
	...
	TS_LOG_INFO("Program firmware from sram to flash, size %zu",
	...
	if (ret) {
		TS_LOG_ERR("Program firmware crc section failed %d", ret);
		return ret;
	}
	...
	if (ret) {
		TS_LOG_ERR("Validate Firmware-CRC section failed %d", ret);
		return ret;
	}

	ret = cts_program_flash(cts_dev, 0, firmware_info->firmware_sect, 4);
	if (ret) {
		TS_LOG_ERR("Program firmware section fist 4bytes failed %d", ret);
		return ret;
	}
	...
	if (ret) {
		TS_LOG_ERR("Validate firmware section failed %d", ret);
		return ret;
	}
		...
		const struct cts_firmware_sect_info *firmware_info)
{
	...
	TS_LOG_INFO("Program DDIParam size: %zu", firmware_info->ddiparam_sect_size);

	ret = cts_program_flash(cts_dev, CTS_DDIPARAM_SECTION_OFFSET,
			firmware_info->ddiparam_sect, firmware_info->ddiparam_sect_size);
	if (ret) {
		TS_LOG_ERR("Program DDIParam section failed %d", ret);
		return ret;
	}
	...
	if (ret) {
		TS_LOG_ERR("Validate DDIParam section failed %d", ret);
		return ret;
	}
	...
	if (ret) {
		TS_LOG_ERR("Program DDIParam-CRC section failed %d", ret);
		return ret;
	}
	...
	if (ret) {
		TS_LOG_ERR("Validate DDIParam-CRC section failed %d", ret);
		return ret;
	}
		...
		const struct cts_firmware *firmware, bool to_flash)
{
	...
	if (parse_firmware(firmware, &firmware_info)) {
		TS_LOG_ERR("Parse firmware failed");
		return -EINVAL;
	}
	...
	TS_LOG_INFO("Update firmware to sram, size %zu, crc 0x%08x",
		firmware_info.firmware_sect_size,
	...
	if ((ret = cts_sram_writel_retry(cts_dev, 0x00FFFC,
		...
		TS_LOG_ERR("Write CRC to 0x0FFFC failed %d", ret);
		goto out;
	}
	if ((ret = cts_sram_writel_retry(cts_dev, 0x00FFF8,
		firmware_info.firmware_sect_size, 3, 1)) != 0) {
		TS_LOG_ERR("Write LEN to 0x0FFF8 failed %d", ret);
		goto out;
	}
	...
	if ((ret = cts_hw_reg_writel_retry(cts_dev, 0x015FF0,
			0xCC33555A, 3, 1)) != 0) {
		TS_LOG_ERR("Write VALID flag to 0x015FF0 failed %d", ret);
		goto out;
	}
	if ((ret = cts_hw_reg_writel_retry(cts_dev, 0x015FF8,
			firmware_info.firmware_sect_crc, 3, 1)) != 0) {
		TS_LOG_ERR("Write CRC to 0x015FF8 failed %d", ret);
		goto out;
	}
	if ((ret = cts_hw_reg_writel_retry(cts_dev, 0x015FFC,
			firmware_info.firmware_sect_size, 3, 1)) != 0) {
		TS_LOG_ERR("Write LEN to 0x015FFC failed %d", ret);
		goto out;
	}
	...
	if (to_flash && !cts_dev->rtdata.has_flash) {
		TS_LOG_ERR("Update firmware to flash is UNKNOWN/NON-EXIST");
		ret = -ENODEV;
	...
	do {
		...
		ret = cts_erase_flash(cts_dev, CTS_FIRMWARE_CRC_SECTION_OFFSET,
				firmware_info.firmware_crc_sect_size);
		if (ret) {
			TS_LOG_ERR("Erase firmware crc section failed %d retries %d",
				ret, retries);
			continue;
		}
		...
		if (ret) {
			TS_LOG_ERR("Erase firmware section failed %d retries %d",
				ret, retries);
			continue;
		}
		...
		if (ret) {
			TS_LOG_ERR("Program firmware & crc section failed %d retries %d",
				ret, retries);
		}
	} while (ret && retries < 3);
	...
	if (ret == 0 && firmware_info.ddiparam_sect_size != 0) {
		...
		do {
			...
			ret = cts_erase_flash(cts_dev, CTS_DDIPARAM_CRC_SECTION_OFFSET,
					firmware_info.ddiparam_crc_sect_size);
			if (ret) {
				TS_LOG_ERR("Erase DDIParam crc secction failed %d, retries %d",
					ret, retries);
				continue;
			}
			...
			if (ret) {
				TS_LOG_ERR("Erase DDIParam section failed %d, retries %d",
					ret, retries);
				continue;
			}
			...
			if (ret) {
				TS_LOG_ERR("Program DDIParam & crc section failed %d retries %d",
					ret, retries);
			}
		} while (ret && retries < 3);
	}
