	...
	uint16_t length, uint16_t limit_len, uint8_t toretry)
{
	...
	if (length > limit_len) {
		TS_LOG_ERR("i2c_nc_read: i2c_read_block size error %d, over array size%d\n",
			length, limit_len);
		return retval;
	}
	...
	if (retval < 0) {
		mdelay(HX_SLEEP_5MS);
		TS_LOG_ERR("i2c_nc_read: i2c_read_block error %d, retry over %d\n",
			retval, toretry);
	}
	return retval;
}
	...
	uint16_t length, uint16_t limit_len, uint8_t toretry)
{
	...
	if ((length > limit_len) || (length >= HX_I2C_MAX_SIZE)) {
		TS_LOG_ERR("i2c_nc_write: i2c_write_block size error %d, over array size%d\n",
			length, limit_len);
		return retval;
	}
	bops = g_himax_nc_ts_data->tskit_himax_data->ts_platform_data->bops;
	...
	if (retval < 0) {
		mdelay(HX_SLEEP_5MS);
		TS_LOG_ERR("i2c_nc_write: i2c_write_block error %d, retry over %d\n",
			retval, toretry);
	}
	return retval;
}
...
void himax_burst_enable(uint8_t auto_add_4_byte)
{
	...
	if (i2c_himax_nc_write(ADDR_EN_BURST_MODE,
		tmp_data, ONE_BYTE_CMD, sizeof(tmp_data),
		DEFAULT_RETRY_CNT) < 0) {
		TS_LOG_ERR("burst_enable: i2c access fail\n");
		return;
	}
	...
	uint8_t *write_data, size_t cnta, size_t cntb)
{
	...
	if (i2c_himax_nc_write(ADDR_FLASH_BURNED, data_byte,
		2 * FOUR_BYTE_CMD, sizeof(data_byte), DEFAULT_RETRY_CNT) < 0) {
		TS_LOG_ERR("flash_write_burst: i2c access fail\n");
		return;
	}

}

void himax_nc_flash_write_burst_length(uint8_t *reg_byte,
	uint8_t *write_data, int length)
{
	...
	if (i2c_himax_nc_write(ADDR_FLASH_BURNED,
		data_byte, length + 4, sizeof(data_byte),
		DEFAULT_RETRY_CNT) < 0) {
		TS_LOG_ERR("flash_write_burst_length: i2c access fail\n");
		return;
	}
}

void himax_nc_register_read(uint8_t *read_addr, int read_length,
	uint8_t *read_data)
{
	uint32_t i;
	uint32_t address;
	...
	if (read_length > MAX_READ_LENGTH) {
		TS_LOG_ERR("nc_register_read: read len over 256\n");
		return;
	}
	...
	if (i2c_himax_nc_write(ADDR_FLASH_BURNED, tmp_data,
		FOUR_BYTE_CMD, sizeof(tmp_data), DEFAULT_RETRY_CNT) < 0) {
		TS_LOG_ERR("nc_register_read: i2c access fail\n");
		return;
	}
	tmp_data[0] = DATA_READ_ACCESS;
	if (i2c_himax_nc_write(ADDR_READ_ACCESS,
		tmp_data, ONE_BYTE_CMD, sizeof(tmp_data),
		DEFAULT_RETRY_CNT) < 0) {
		TS_LOG_ERR("nc_register_read: i2c access fail\n");
		return;
	...
	if (i2c_himax_nc_read(DUMMY_REGISTER, read_data,
		read_length, MAX_READ_LENGTH, DEFAULT_RETRY_CNT) < 0) {
		TS_LOG_ERR("nc_register_read: i2c access fail\n");
		return;
	}
	if (read_length > FOUR_BYTE_CMD)
...
void himax_register_write(uint8_t *write_addr,
	int write_length, uint8_t *write_data)
{
	...
	for (i = address; i < address + write_length; i++) {
		...
		himax_nc_flash_write_burst_length(write_addr, write_data, write_length);
	}
}
