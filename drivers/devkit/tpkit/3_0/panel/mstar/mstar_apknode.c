                          ...
                          loff_t * pPos)
{
	...
	if (pBuffer != NULL) {
		ret = copy_from_user(cmd, pBuffer, APKNODE_WRITE_MESSAGE_LENGTH);
		if (ret < 0) {
                        ...
                        loff_t * pPos)
{
    ...
    if (nBusType == SLAVE_I2C_ID_DBBUS || nBusType == SLAVE_I2C_ID_DWI2C) {
        ...
        nRet = copy_from_user(buffer, pBuffer, APKNODE_MSGTOOL_WRITE_LENGTH);
        if (nRet < 0) {
...
static ssize_t ts_info_show(struct class *class, struct class_attribute *attr, char *buf)
{
    return sprintf(buf, "mstar ts_info_show test");
}

static CLASS_ATTR_RO(ts_info);
...
static ssize_t mstar_proc_debug_message_write(struct file *filp, const char *buff, size_t size, loff_t *pPos)
{
	...
	ret = copy_from_user(buffer, buff, APKNODE_WRITE_MESSAGE_LENGTH);
	if (ret < 0) {
		TS_LOG_ERR("copy data from user space, failed");
		return -1;
	}
