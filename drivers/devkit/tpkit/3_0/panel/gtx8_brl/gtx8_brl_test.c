...
static int brl_reg_write_confirm(u32 addr, u8 *data, u32 len)
{
	u8 *cfm;
	int r;
	int i;
	...
	for (i = 0; i < GTX8_RETRY_NUM_3; i++) {
		r = brl_ts->ops.i2c_write(addr, data, len);
		if (r < 0)
			goto exit;

		r = brl_ts->ops.i2c_read(addr, cfm, len);
		if (r < 0)
			goto exit;
...
static int brl_load_mbist(struct brl_ts_test *ts_test)
{
	...
	r = brl_reg_write_confirm(ISP_RAM_ADDR,
		(u8 *)ts_test->firmware->data, ISP_BANK0_SIZE);
	if (r < 0) {
		TS_LOG_ERR("Loading mbist write bank0 error\n");
		return r;
	}
	r = brl_reg_write_confirm(ISP_MBIST_ADDR,
