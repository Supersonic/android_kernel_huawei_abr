...
static unsigned char *chip_roi_rawdata(void)
{
	...
	if (ilits->roi_completion_timeout_for_pk)
		timeout = ROI_WAIT_TIMEOUT_PK;
	else
		timeout = ROI_WAIT_TIMEOUT;
	if (wait_for_completion_interruptible_timeout(&ilits->roi_completion, msecs_to_jiffies(timeout))) {
