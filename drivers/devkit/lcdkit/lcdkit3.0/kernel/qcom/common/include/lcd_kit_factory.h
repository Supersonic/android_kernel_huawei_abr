...
#define LCD_RSP_STR_LENGTH_MAX	100
...
struct lcd_fact_ops {
	...
	ssize_t (*oneside_mode_show)(struct device *dev,
		struct device_attribute *attr, char *buf);
	ssize_t (*fpc_det_show)(struct device *dev,
		struct device_attribute *attr, char *buf);
};
...
struct poweric_detect {
	...
	u32 poweric_num_length;
	u32 poweric_status[POWERIC_NUM_MAX];
	struct lcd_kit_array_data gpio_list;
	struct lcd_kit_array_data gpio_val;
	struct lcd_kit_array_data gpio_num_list;
};

struct fpc_detect_desc {
	u32 support;
	u32 pinctrl_cfg;
	struct lcd_kit_array_data det_gpio_list;
	struct lcd_kit_array_data exp_gpio_val_list;
};
...
struct lcd_fact_desc {
	...
	/* fpc detect */
	struct fpc_detect_desc fpc_detect;
};
...
int lcd_kit_avdd_mipi_ctrl(uint32_t panel_id, void *hld, int enable);
