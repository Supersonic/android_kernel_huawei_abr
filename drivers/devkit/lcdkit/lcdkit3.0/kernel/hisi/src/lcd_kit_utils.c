...
enum {
	...
	GAMMA_INDEX_GREEN_HIGH = 2,
	GAMMA_INDEX_GREEN_LOW = 3,
	GAMMA_INDEX_BLUE_HIGH = 4,
	GAMMA_INDEX_BLUE_LOW = 5
};
...
static int lcd_kit_write_local_hbm_gamma_by_scene_no_lock(
	struct hisi_fb_data_type *hisifd, const int index)
{
	int ret;
	...
	if (index == LCD_FPS_SCENCE_60P) {
		/* R */
		common_info->hbm.circle_color_cmds.cmds[2].payload[1] =
			common_info->hbm.hbm_gamma.red_60_hz[0];
		common_info->hbm.circle_color_cmds.cmds[2].payload[2] =
			common_info->hbm.hbm_gamma.red_60_hz[1];
		/* G */
		common_info->hbm.circle_color_cmds.cmds[4].payload[1] =
			common_info->hbm.hbm_gamma.green_60_hz[0];
		common_info->hbm.circle_color_cmds.cmds[4].payload[2] =
			common_info->hbm.hbm_gamma.green_60_hz[1];
		/* B */
		common_info->hbm.circle_color_cmds.cmds[6].payload[1] =
			common_info->hbm.hbm_gamma.blue_60_hz[0];
		common_info->hbm.circle_color_cmds.cmds[6].payload[2] =
			common_info->hbm.hbm_gamma.blue_60_hz[1];
	}
	if (index == LCD_FPS_SCENCE_90) {
		/* R */
		common_info->hbm.circle_color_cmds.cmds[2].payload[1] =
			common_info->hbm.hbm_gamma.red_90_hz[0];
		common_info->hbm.circle_color_cmds.cmds[2].payload[2] =
			common_info->hbm.hbm_gamma.red_90_hz[1];
		/* G */
		common_info->hbm.circle_color_cmds.cmds[4].payload[1] =
			common_info->hbm.hbm_gamma.green_90_hz[0];
		common_info->hbm.circle_color_cmds.cmds[4].payload[2] =
			common_info->hbm.hbm_gamma.green_90_hz[1];
		/* B */
		common_info->hbm.circle_color_cmds.cmds[6].payload[1] =
			common_info->hbm.hbm_gamma.blue_90_hz[0];
		common_info->hbm.circle_color_cmds.cmds[6].payload[2] =
			common_info->hbm.hbm_gamma.blue_90_hz[1];
	}
	ret = lcd_kit_dsi_cmds_tx_no_lock(hisifd,
...
static int lcd_kit_updt_panel_fps(struct hisi_fb_data_type *hisifd, char fps_scence)
{
	...
	lcd_kit_write_local_hbm_gamma_by_scene_no_lock(hisifd, index);
...
int lcd_kit_local_hbm_alpha_circle_with_alpha_and_dbv(
	struct hisi_fb_data_type *hisifd, bool is_on, uint32_t alpha, uint32_t dbv)
{
	...
	if (!is_on) {
		...
		lcd_kit_exit_ddic_alpha(hisifd, LCD_KIT_ALPHA_DEFAULT);
		lcd_kit_local_hbm_set_circle(hisifd, false);
	} else {
		if (alpha > LCD_KIT_ALPHA_DEFAULT)
			alpha = LCD_KIT_ALPHA_DEFAULT;

		LCD_KIT_DEBUG("alpha circle enter\n");
		lcd_kit_enter_ddic_alpha(hisifd, alpha);
		lcd_kit_local_hbm_set_dbv(hisifd, dbv);
		lcd_kit_local_hbm_set_circle(hisifd, true);
	}
	return LCD_KIT_OK;
}

int lcd_kit_local_hbm_set_circle(struct hisi_fb_data_type *hisifd, bool is_on)
{
	int ret;
	if (!hisifd) {
		LCD_KIT_ERR("hisifd is null\n");
...
int lcd_kit_local_hbm_set_dbv(struct hisi_fb_data_type *hisifd, uint32_t dbv)
{
	int ret;
	...
	// hbm_dbv_cmds.cmd_cnt's maximum value is 3
	if (common_info->hbm.hbm_dbv_cmds.cmd_cnt != 3) {
		LCD_KIT_ERR("hbm_dbv_cmds.cmd_cnt does not match");
		return LCD_KIT_FAIL;
	}
	common_info->hbm.hbm_dbv_cmds.cmds[2].payload[1]
		= ((dbv >> 8) & 0x0f);
	common_info->hbm.hbm_dbv_cmds.cmds[2].payload[2]
		= dbv & 0xff;
	ret = lcd_kit_dsi_cmds_tx(hisifd,
...
void lcd_kit_hbm_gamma_read(void *hld)
{
	...
	common_info->hbm.hbm_gamma.red_60_hz[0] = hbm_gamma[GAMMA_INDEX_RED_HIGH];
	common_info->hbm.hbm_gamma.red_60_hz[1] = hbm_gamma[GAMMA_INDEX_RED_LOW];
	common_info->hbm.hbm_gamma.green_60_hz[0] = hbm_gamma[GAMMA_INDEX_GREEN_HIGH];
	common_info->hbm.hbm_gamma.green_60_hz[1] = hbm_gamma[GAMMA_INDEX_GREEN_LOW];
	common_info->hbm.hbm_gamma.blue_60_hz[0] = hbm_gamma[GAMMA_INDEX_BLUE_HIGH];
	common_info->hbm.hbm_gamma.blue_60_hz[1] = hbm_gamma[GAMMA_INDEX_BLUE_LOW];

	for (i = 0; i <= GAMMA_INDEX_BLUE_LOW; i++)
	...
	common_info->hbm.hbm_gamma.red_90_hz[0] = hbm_gamma[GAMMA_INDEX_RED_HIGH];
	common_info->hbm.hbm_gamma.red_90_hz[1] = hbm_gamma[GAMMA_INDEX_RED_LOW];
	common_info->hbm.hbm_gamma.green_90_hz[0] = hbm_gamma[GAMMA_INDEX_GREEN_HIGH];
	common_info->hbm.hbm_gamma.green_90_hz[1] = hbm_gamma[GAMMA_INDEX_GREEN_LOW];
	common_info->hbm.hbm_gamma.blue_90_hz[0] = hbm_gamma[GAMMA_INDEX_BLUE_HIGH];
	common_info->hbm.hbm_gamma.blue_90_hz[1] = hbm_gamma[GAMMA_INDEX_BLUE_LOW];
	...
	size_t count)
{
	...
	case DDIC_LOCAL_HBM:
		...
		ret = lcd_kit_local_hbm_alpha_circle_with_alpha_and_dbv(hisifd,
			(bool)param[1], (uint32_t)param[2], (uint32_t)param[3]);
		break;
	default:
		break;
	}
