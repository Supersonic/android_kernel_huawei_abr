...
int lcd_kit_enter_ddic_alpha(struct hisi_fb_data_type *hisifd, uint32_t alpha)
{
	int ret = LCD_KIT_OK;
	int payload_num;
	int cmd_cnt;
	...
	if (cmd_cnt <= 0 || cmd_cnt > cmd_cnt_max) {
		LCD_KIT_ERR("enter_alpha_cmds's cmd_cnt is invalid, cmd_cnt:%d", cmd_cnt);
		return LCD_KIT_FAIL;
	}
	...
	if (payload_num < payload_num_min || payload_num > payload_num_max) {
		LCD_KIT_ERR("enter_alpha_cmds.cmds's payload_num is invalid, payload_num:%d", payload_num);
		return LCD_KIT_FAIL;
	}
	...
	if (common_info->ddic_alpha.alpha_with_enable_flag == 0)
		common_info->ddic_alpha.enter_alpha_cmds.cmds[cmd_cnt - 1].
			payload[payload_num - 2] = (alpha >> 8) & 0x0f;
	else
		common_info->ddic_alpha.enter_alpha_cmds.cmds[cmd_cnt - 1].
			payload[payload_num - 2] = ((alpha >> 8) & 0x0f) | 0x10;

	/* Sets the lower eight bits of the alpha. */
	common_info->ddic_alpha.enter_alpha_cmds.cmds[cmd_cnt - 1].
		payload[payload_num - 1] = alpha & 0xff;
	ret = lcd_kit_dsi_cmds_tx(hisifd, &common_info->ddic_alpha.enter_alpha_cmds);
	if (ret != LCD_KIT_OK)
...
u32 get_local_hbm_support(void)
{
