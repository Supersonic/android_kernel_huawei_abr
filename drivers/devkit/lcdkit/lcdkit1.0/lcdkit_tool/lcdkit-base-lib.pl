...
sub create_hisi_lcd_config_data
{
    ...
    $lcd_config .= "        pinfo->hbm_enter_cmds.cmds_set = ".lc($lcd_name)."_hbm_enter_cmds;\n";
    $lcd_config .= "        pinfo->hbm_enter_cmds.cmd_cnt  = ARRAY_SIZE(".lc($lcd_name)."_hbm_enter_cmds);\n";
    $lcd_config .= "        pinfo->hbm_exit_cmds.cmds_set = ".lc($lcd_name)."_hbm_exit_cmds;\n";
    $lcd_config .= "        pinfo->hbm_exit_cmds.cmd_cnt  = ARRAY_SIZE(".lc($lcd_name)."_hbm_exit_cmds);\n";
    $lcd_config .= "        pinfo->aod_hbm_enter_cmds.cmds_set = ".lc($lcd_name)."_aod_hbm_enter_cmds;\n";
    $lcd_config .= "        pinfo->aod_hbm_enter_cmds.cmd_cnt  = ARRAY_SIZE(".lc($lcd_name)."_aod_hbm_enter_cmds);\n";
    $lcd_config .= "        pinfo->aod_hbm_exit_cmds.cmds_set = ".lc($lcd_name)."_aod_hbm_exit_cmds;\n";
    $lcd_config .= "        pinfo->aod_hbm_exit_cmds.cmd_cnt  = ARRAY_SIZE(".lc($lcd_name)."_aod_hbm_exit_cmds);\n";
...
sub create_hisi_panel_update
{
    my $panel_init;
    $panel_init  = "static uint8_t lcdkit_panel_update(uint16_t product_id, uint16_t lcd_id)\n";
    $panel_init .= "{\n";
    $panel_init .= "    uint8_t lcd_panel_id = " . $default_panel . ";\n";
    $panel_init .= "    uint32_t i = 0;\n";

    $panel_init .= "    for (i = 0; i < ARRAY_SIZE(lcdkit_map); ++i) {\n";
    $panel_init .= "        if (lcdkit_map[i].board_id == product_id) {\n";
    $panel_init .= "            if (lcdkit_map[i].gpio_id == 0) {\n";
    $panel_init .= "                lcd_panel_id = lcdkit_map[i].lcd_id;\n";
    $panel_init .= "            }\n";
    $panel_init .= "            if (lcdkit_map[i].gpio_id == lcd_id) {\n";
    $panel_init .= "                lcd_panel_id = lcdkit_map[i].lcd_id;\n";
    $panel_init .= "                break;\n";
    $panel_init .= "            }\n";
    $panel_init .= "        }\n";
