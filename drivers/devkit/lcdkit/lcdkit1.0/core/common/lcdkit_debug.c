							...
							char __user *buff, size_t count,loff_t *ppos)
{
	...
	{
		ret_len = snprintf_s(lcd_debug_buf, sizeof(lcd_debug_buf), sizeof(lcd_debug_buf) - 1,
			"reg value is invalid! Read first!\n");
		if (ret_len < 0)
			LCDKIT_ERR("reg value is invalid! Read first!\n");
	}
	...
	else
	{
		...
		if(ret)
		{
			...
			if (ret_len < 0)
				LCDKIT_ERR("lcdkit_ic_mipi_reg read invalid!\n");
		}
		else
		{
			...
			for (i = 0; i < lcdkit_dbg.lcdkit_g_param_num; i++) {
				...
				case 0:
					if (i == lcdkit_dbg.lcdkit_g_param_num - 1)
						ret_val = snprintf_s(tmp, sizeof(tmp), sizeof(tmp) - 1, "0x%02x",
							(uint32_t)lcdkit_dbg.lcdkit_ic_mipi_value[i / INT_COUNT] & 0xFF);
					else
						ret_val = snprintf_s(tmp, sizeof(tmp), sizeof(tmp) - 1, "0x%02x,",
							(uint32_t)lcdkit_dbg.lcdkit_ic_mipi_value[i / INT_COUNT] & 0xFF);
					if (ret_val < 0)
						LCDKIT_ERR("lcdkit_ic_mipi_value 0 error!\n");
					break;
				case 1:
					if (i == lcdkit_dbg.lcdkit_g_param_num - 1)
						ret_val = snprintf_s(tmp, sizeof(tmp), sizeof(tmp) - 1, "0x%02x",
							((uint32_t)lcdkit_dbg.lcdkit_ic_mipi_value[i / INT_COUNT] >> REG_BIT_COUNT) & 0xFF);
					else
						ret_val = snprintf_s(tmp, sizeof(tmp), sizeof(tmp) - 1, "0x%02x,",
							((uint32_t)lcdkit_dbg.lcdkit_ic_mipi_value[i / INT_COUNT] >> REG_BIT_COUNT) & 0xFF);
					if (ret_val < 0)
						LCDKIT_ERR("lcdkit_ic_mipi_value 1  error!\n");
					break;
				case 2:
					if (i == lcdkit_dbg.lcdkit_g_param_num - 1)
						ret_val = snprintf_s(tmp, sizeof(tmp), sizeof(tmp) - 1, "0x%02x",
							((uint32_t)lcdkit_dbg.lcdkit_ic_mipi_value[i / INT_COUNT] >> (2 * REG_BIT_COUNT)) & 0xFF);

					else
						ret_val = snprintf_s(tmp, sizeof(tmp), sizeof(tmp) - 1, "0x%02x,",
							((uint32_t)lcdkit_dbg.lcdkit_ic_mipi_value[i / INT_COUNT] >> (2 * REG_BIT_COUNT)) & 0xFF);
					if (ret_val < 0)
						LCDKIT_ERR("lcdkit_ic_mipi_value 2  error!\n");
					break;
				case 3:
					if (i == lcdkit_dbg.lcdkit_g_param_num - 1)
						ret_val = snprintf_s(tmp, sizeof(tmp), sizeof(tmp) - 1, "0x%02x",
							((uint32_t)lcdkit_dbg.lcdkit_ic_mipi_value[i / INT_COUNT] >> (3 * REG_BIT_COUNT)) & 0xFF);
					else
						ret_val = snprintf_s(tmp, sizeof(tmp), sizeof(tmp) - 1, "0x%02x,",
							((uint32_t)lcdkit_dbg.lcdkit_ic_mipi_value[i / INT_COUNT] >> (3 * REG_BIT_COUNT)) & 0xFF);
					if (ret_val < 0)
						LCDKIT_ERR("lcdkit_ic_mipi_value 3  error!\n");
					break;
				}
				strncat(lcd_debug_buf, tmp, strlen(tmp));
			}
			ret_len = snprintf_s(tmp, sizeof(tmp), sizeof(tmp) - 1,
				"0x%02x = %s\n", lcdkit_dbg.lcdkit_ic_mipi_reg, lcd_debug_buf);
			if (ret_len < 0)
				LCDKIT_ERR("lcdkit_ic_mipi_reg  error!\n");
			lcdkit_release(ctrl);
		}
