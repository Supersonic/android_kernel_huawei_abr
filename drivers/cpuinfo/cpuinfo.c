...
static void cpuinfo_get_mx_cx_mode(int *mx_mode, int *cx_mode)
{
    ...
    union uni_corner_info corner={{0,0}};
                              ...
                              unsigned long event, void *ptr)
{
    ...
    rc = regmap_read(cpuinfo->regmap, cpuinfo->pmic_para.cx_type_reg, &cx_type);
    if (rc) {
        dev_err(cpuinfo->dev, "Unable to read cx type reg rc=%d\n", rc);
        goto fail;
    }

    rc = regmap_read(cpuinfo->regmap, cpuinfo->pmic_para.cx_value_reg, &cx_value);
    if (rc) {
        dev_err(cpuinfo->dev, "Unable to read cx type reg rc=%d\n", rc);
        goto fail;
    }

    rc = regmap_read(cpuinfo->regmap, cpuinfo->pmic_para.mx_value_reg, &mx_value);
    if (rc) {
        dev_err(cpuinfo->dev, "Unable to read mx value reg rc=%d\n", rc);
        goto fail;
    }

    rc = regmap_read(cpuinfo->regmap, cpuinfo->pmic_para.mss_type_reg, &mss_type);
    if (rc) {
        dev_err(cpuinfo->dev, "Unable to read mss reg rc=%d\n", rc);
        goto fail;
    }

    rc = regmap_read(cpuinfo->regmap, cpuinfo->pmic_para.mss_value_reg, &mss_value);
    if (rc) {
        dev_err(cpuinfo->dev, "Unable to read mss reg rc=%d\n", rc);
        goto fail;
    }
...
static int cpuinfo_get_device_tree_data(struct platform_device *pdev, struct cpu_info_struct *cpuinfo)
{
    int rc = 0;
    u16 temp_reg_data[2];
...
static int cpuinfo_probe(struct platform_device *pdev)
{
    ...
    cpuinfo = devm_kzalloc(&pdev->dev, sizeof(struct cpu_info_struct), GFP_KERNEL);

    if (!cpuinfo) {
        dev_err(&pdev->dev, "Can't allocate cpuinfo\n");
        return -ENOMEM;
    }

    cpuinfo->dev = &pdev->dev;
    cpuinfo->regmap = dev_get_regmap(cpuinfo->dev->parent, NULL);
    if (!cpuinfo->regmap) {
        dev_err(cpuinfo->dev, "Parent regmap is unavailable\n");
        return -ENXIO;
    }

    rc = cpuinfo_get_device_tree_data(pdev, cpuinfo);
    if(rc) {
...
static int cpuinfo_remove(struct platform_device *pdev)
{
...
static struct platform_driver cpuinfo_driver = {
    .probe = cpuinfo_probe,
...
static int __init hw_cpuinfo_init(void)
{
    ...
    err = platform_driver_register(&cpuinfo_driver);
    if (err){
...
{
    platform_driver_unregister(&cpuinfo_driver);
}
