...
static struct batt_uuid_record g_raw_record;
...
void batt_write_bind_mesg(void)
{
	...
	for (i = 0; i < POWER_MESG_RETRY_TIMES; i++) {
		...
		if (!g_bind_info.update_sign ||
			!memcmp(&g_raw_record, &g_bind_info.bind_info.record,
		...
		wait_for_completion_timeout(&g_bind_info.write_bind_end,
			POWER_MESG_TIME_OUT);
		memcpy(&g_raw_record, &g_bind_info.bind_info.record,
			sizeof(struct batt_uuid_record));
