...
{
	struct stwlc33_device_info *di = g_stwlc33_di;
	u8 buff[STWLC33_ADDR_LEN + POWER_BYTE_LEN] = { 0 };
	...
	return stwlc33_write_block(di, reg, buff, POWER_BYTE_LEN);
}

static int stwlc33_write_word(u16 reg, u16 data)
{
	struct stwlc33_device_info *di = g_stwlc33_di;
	u8 buff[STWLC33_ADDR_LEN + POWER_WORD_LEN] = { 0 };
	...
	buff[STWLC33_ADDR_LEN] = data & POWER_MASK_BYTE;
	buff[STWLC33_ADDR_LEN + 1] = data >> POWER_BITS_PER_BYTE;
...
static int stwlc33_send_fsk_msg_tx_cap(void)
{
	...
	if (wlrx_get_wired_channel_state() == WIRED_CHANNEL_OFF)
		tx_cap_data[TX_CAP_TYPE] = WLACC_ADP_OTG_A;
	else
		tx_cap_data[TX_CAP_TYPE] = WLACC_ADP_OTG_B;
...
static void stwlc33_handle_ask_packet(struct stwlc33_device_info *di)
{
	...
	case STWLC33_CMD_GET_TX_ID:
		tx_id = (packet_data[2] << POWER_BITS_PER_BYTE) | packet_data[3];
		if (tx_id == TX_ID_HW) {
...
static void stwlc33_tx_mode_irq_handler(struct stwlc33_device_info *di)
{
	...
	if (ret) {
		...
		stwlc33_clear_tx_interrupt(POWER_MASK_WORD);
		goto func_end;
	}
