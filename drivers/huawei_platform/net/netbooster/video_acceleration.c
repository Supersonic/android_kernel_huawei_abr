 ...
 * Description: This file is used by the netfilter hook for video
...
{
	unsigned long speed_x128 = 0;
	unsigned long speed_norm0 = 0;
	unsigned long speed_norm = 0;
	unsigned int err_pkt = 0;
...
static unsigned int bit_rate(unsigned int idx)
{
	...
	g_filter.talp = g_filter.talp - g_filter.talp / MULTIPLIER_X20 +
		g_stat.stat[idx].tin_len / MULTIPLIER_X20;

	g_filter.ualp = g_filter.ualp - g_filter.ualp / MULTIPLIER_X20 +
		g_stat.stat[idx].uin_len / MULTIPLIER_X20;
	...
	if (g_filter.talp < g_filter.ualp &&
		...
		g_bit_rate = g_filter.ualp + g_filter.ualp / MULTIPLIER_X2;
	}
	chr_stat(is_freez);
	return 0;
}
...
static unsigned int idx_update(void)
{
	...
	if (index != g_stat.idx || time_stamp != g_stat.time_stamp) {
		for (cnt = 1; cnt <= time_stamp - g_stat.time_stamp; cnt++) {
			...
			/* Current packet is integersecond. speed to be count */
				bit_rate(index_tmp);
				break;
			}
			bit_rate(index_tmp);
...
static int tcp_in_cnt(struct tcp_sock *sk, unsigned int len, unsigned int req)
{
	 ...
	 * the current packet Req-len can calculate the first byte of the
	 * packet req, if larger than RCV_NXT, This packet is a messy packet.
	 */
...
static int u_stream_info(unsigned short port, unsigned int ip,
	unsigned int len)
{
	int cnt;
	int max_num;
	...
	for (cnt = 0; cnt < PKT_VECTOR_LEN; cnt++) {
		if (before(g_stream.u_time_stamp[cnt] + STREAM_END_TIMER,
			jiffies)) {
...
static unsigned int report_stat(void)
{
	...
	if (g_video_stat.t_u_flag == STREAM_TCP) {
		...
		g_report.seg_duration = DURATION_TIME;
		g_report.video_ip = (g_stream.t_ip[g_stream.t_num] | IP_MASK);
		g_report.video_port = g_stream.t_port[g_stream.t_num];
		g_report.video_status = g_video_stat.t_stat;
	} else {
		g_stream.u_num = g_stream.u_num % PKT_VECTOR_LEN;
		g_report.seg_duration = DURATION_TIME;
		g_report.video_ip = (g_stream.u_ip[g_stream.t_num] | IP_MASK);
		g_report.video_port = g_stream.u_port[g_stream.t_num];
		g_report.video_status = g_video_stat.u_stat;
	}
	g_report.ave_code_rate = g_bit_rate;
	g_report.seg_index = g_seg.seg_idx++;
	g_report.video_protocol = VIDEO_PROTOCOL_HLS;
	g_report.video_remaining_playtime = g_seg.seg_remain;
	g_report.video_seg_state = CHOICE_RESULT_VIDEO_CHANGE;

	nb_notify_event(NBMSG_VOD_EVT, &g_report, sizeof(g_report));
	pr_info("VOD media report status:%d\n", g_report.video_status);
...
static int t_video_state(unsigned char *pstr, unsigned int len,
	...
	for (cnt = 0; cnt < len - CONTENT_TYPE_LEN; cnt++) {
		if (pstr[cnt] == ASCII_CR && pstr[cnt + 1] == ASCII_LF) {
			...
			if (cnt >= len - CONTENT_TYPE_LEN) {
				break;
			} else if (pstr[cnt] == ASCII_CR && pstr[cnt + 1] ==
				ASCII_LF) {
				break;
			} else if (strncmp(&pstr[cnt],
				CONTENT_TYPE1, CONTENT_TYPE_LEN) == 0 ||
				strncmp(&pstr[cnt], CONTENT_TYPE2,
				CONTENT_TYPE_LEN) == 0) {
				modify_status();
...
#else
static void report_seg(struct timer_list* data)
#endif
{
	spin_lock_bh(&g_video_lock);
	...
	if (g_video_stat.t_stat != VIDEO_STATUS_END &&
		before(g_video_stat.t_time_stamp + SWITCH_END_TIMER, jiffies))
		g_video_stat.t_stat = VIDEO_STATUS_END;
	...
	if (g_video_stat.t_u_flag == STREAM_TCP) {
		...
		g_stream.u_num = g_stream.u_num % PKT_VECTOR_LEN;
		g_report.seg_duration = DURATION_TIME;
		g_report.video_ip = (g_stream.u_ip[g_stream.u_num] | IP_MASK);
		g_report.video_port = g_stream.u_port[g_stream.u_num];
		g_report.video_status = g_video_stat.u_stat;
	}
	g_report.ave_code_rate = g_bit_rate;
	g_report.seg_index = g_seg.seg_idx++;
	g_report.video_protocol = VIDEO_PROTOCOL_HLS;
	g_report.video_remaining_playtime = g_seg.seg_remain;
	g_report.video_seg_state = CHOICE_RESULT_VIDEO_SEG;

	nb_notify_event(NBMSG_VOD_EVT, &g_report, sizeof(g_report));
	pr_info("VOD media report segment:%d\n", g_report.ave_code_rate);
	...
	const struct nf_hook_state *state)
{
	...
	if (skb == NULL || g_hook_flag == HOOK_DISABLE)
		return NF_ACCEPT;

	iph = ip_hdr(skb);
	if (iph == NULL || skb->dev == NULL || skb->dev->name == NULL ||
		strncmp(skb->dev->name, VIDEO_DS_NET, VIDEO_DS_NET_LEN))
		return NF_ACCEPT;
	...
	if (iph->protocol == IPPROTO_TCP) {
		...
		if (total_len > MAX_HTTP_LEN || ip_hdr_len > MAX_HTTP_LEN ||
			data_len > MAX_HTTP_LEN)
			return NF_ACCEPT;

		if (htons(tcph->dest) != VIDEO_HTTP_PORT || skb->sk == NULL ||
		...
		udph = udp_hdr(skb);
		if (udph == NULL || !spin_trylock_bh(&g_video_lock))
			return NF_ACCEPT;
	...
	const struct nf_hook_state *state)
{
	...
	if (iph->protocol == IPPROTO_TCP) {
		...
		if (total_len > MAX_HTTP_LEN || ip_hdr_len > MAX_HTTP_LEN ||
			data_len > MAX_HTTP_LEN || htons(tcph->source) !=
			VIDEO_HTTP_PORT || skb->sk == NULL)
			return NF_ACCEPT;

		if (skb->sk->sk_state != TCP_ESTABLISHED || !spin_trylock_bh(&g_video_lock))
			return NF_ACCEPT;

		idx_update();
		tcp_in_cnt((struct tcp_sock *)skb->sk, data_len, ntohl(tcph->seq));
		t_video_state(tcp_data, data_len, tcph->dest, iph->saddr);

		spin_unlock_bh(&g_video_lock);
	} else if (iph->protocol == IPPROTO_UDP) {
		total_len = skb->len - skb->data_len;
		udph = udp_hdr(skb);
		if (udph == NULL || total_len > MAX_HTTP_LEN ||
			!spin_trylock_bh(&g_video_lock))
			return NF_ACCEPT;

		if (idx_update() > 0)
			u_video_state();
		u_stream_info(udph->dest, iph->saddr, total_len);
...
#else
	timer_setup(&g_seg_rpt_timer, report_seg, 0);
#endif
