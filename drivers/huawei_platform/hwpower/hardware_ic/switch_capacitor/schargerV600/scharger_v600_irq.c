				 ...
				 char *buf_addr, int *offset, const char *buf)
{
	...
	if (strlen(g_buck_irq_str[index]) <
		...
		ret = snprintf(buf_addr + (*offset), str_len -
				 strlen(buf), "%s", g_buck_irq_str[index]);
		if (ret < 0) {
				...
				char *buf_addr, int *offset, const char *buf)
{
	...
	if (strlen(g_lvc_sc_irq_str[index]) <
		(str_len - strlen(buf_addr))) {
		ret = snprintf(buf_addr + (*offset), str_len - strlen(buf),
...
static void hi6526_lvc_sc_irq2str(u32 lvc_sc_irq_state)
{
	...
	offset = snprintf(buf_addr, str_len,
...
static void hi6526_chip_overtemp_handle(struct hi6526_device_info *di)
{
	...
	hi6526_direct_dmd_report(di, POWER_NE_DC_FAULT_TDIE_OTP, data);
}
...
static void hi6526_irq_clear(struct hi6526_device_info *di)
{
	...
	ret = ret || hi6526_read(di, CHG_PD_ALERT_L_ADDR, &val);
	if (val) {
		scharger_err("[%s]: irq[0x10]=0x%x\n", __func__, val);
		ret = ret || hi6526_write(di, CHG_PD_ALERT_L_ADDR, val);
	}

	ret = ret || hi6526_read(di, CHG_PD_ALERT_H_ADDR, &val);
	if (val) {
		scharger_err("[%s]: irq[0x11]=0x%x\n", __func__, val);
		ret = ret || hi6526_write(di, CHG_PD_ALERT_H_ADDR, val);
	}
