				       ...
				       unsigned long event, void *data)
{
	...
	struct cpufreq_policy_data policy_data;
	struct drg_freq_device *freq_dev = NULL;
	unsigned long max_freq;
	unsigned long min_freq = 0;

	if (event != CPUFREQ_CREATE_POLICY || !g_drg_initialized)
		return NOTIFY_DONE;
	...
	memcpy_s(&policy_data.cpuinfo, sizeof(policy_data.cpuinfo), &policy->cpuinfo,
		 sizeof(policy->cpuinfo));
	policy_data.freq_table = policy->freq_table;
	policy_data.cpu = policy->cpu;

	policy_data.min = freq_qos_read_value(&policy->constraints, FREQ_QOS_MIN);
	policy_data.max = freq_qos_read_value(&policy->constraints, FREQ_QOS_MAX);
