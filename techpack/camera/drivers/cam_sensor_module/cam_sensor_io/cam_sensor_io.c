...
#define DW9781_OIS_NAME              "dw9781"

bool avoid_to_report(struct camera_io_master *io_master_info)
{
	struct cam_ois_ctrl_t *o_ctrl = NULL;

	if (io_master_info->device_type == CAM_OIS) {
		o_ctrl = container_of(io_master_info, struct cam_ois_ctrl_t, io_master_info);
		if (strcmp(o_ctrl->ois_name, DW9781_OIS_NAME) == 0)
			return true;
	}
	...
	uint32_t delay_ms)
{
	...
	/* DW9781B and DW9781C not need to report i2c error first time */
	if (rc && !avoid_to_report(io_master_info)) {
