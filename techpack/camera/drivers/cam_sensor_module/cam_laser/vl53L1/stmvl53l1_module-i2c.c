 ...
 */
static int stmvl53l1_parse_tree(struct device *dev, struct i2c_data *i2c_data, struct cam_laser_ctrl_t *o_ctrl)
{
	int rc = 0;
	...
	i2c_data->laser_state = LASER_ALWAYS_ON; // xshut gpio always on
	i2c_data->vdd = NULL;
	i2c_data->pwren_gpio = -1;
	i2c_data->xsdn_gpio = o_ctrl->soc_info.gpio_data->cam_gpio_common_tbl[0].gpio; // EN
	i2c_data->intr_gpio = o_ctrl->soc_info.gpio_data->cam_gpio_common_tbl[1].gpio; // INT
				...
				const struct i2c_device_id *id)
{
	...
	vl53l1_info("Enter stmvl53l1_probe %s : 0x%02x", client->name, client->addr);
	...
	o_ctrl->soc_info.soc_private = (void *)vl53l1_data;
	o_ctrl->soc_info.dev_name = STMVL53L1_DRV_NAME;
	/* parse and configure hardware */
	rc = stmvl53l1_parse_tree(&i2c_data->client->dev, i2c_data, o_ctrl);
	if (rc)
		goto done_freemem;

	/* end up by core driver setup */
	rc = stmvl53l1_setup(vl53l1_data, o_ctrl);
	if (rc) {
		vl53l1_errmsg("probe : stmvl53l1_setup fail");
		goto release_gpios;
	}

	/* match id succeed, then set devcheck flag */
	set_hw_dev_flag(DEV_I2C_LASER);
	vl53l1_info("stmvl53l1_probe succeed");
...
done_freemem:
	...
	kfree(vl53l1_data);
	kfree(i2c_data);
	vl53l1_errmsg("stmvl53l1_probe failed");
...
int stmvl53l1_remove(struct i2c_client *client)
{
	...
	struct stmvl53l1_data *data = o_ctrl->soc_info.soc_private;
	struct i2c_data *i2c_data = (struct i2c_data *)data->client_object;

	vl53l1_dbgmsg("Enter stmvl53l1_remove");
...
{
	struct cam_laser_ctrl_t *o_ctrl = i2c_get_clientdata(to_i2c_client(dev));
	struct stmvl53l1_data *data = o_ctrl->soc_info.soc_private;

	vl53l1_dbgmsg("Enter stmvl53l1_suspend");
	mutex_lock(&data->work_mutex);
	/* Stop ranging */
	...
	vl53l1_dbgmsg("End stmvl53l1_suspend");
...
static int stmvl53l1_resume(struct device *dev)
{
	...
	vl53l1_dbgmsg("End\n");
#else
	vl53l1_dbgmsg("Enter stmvl53l1_resume");
	vl53l1_dbgmsg("End stmvl53l1_resume");
#endif
	return 0;
}
#endif
...
int stmvl53l1_power_up_i2c(void *object)
{
	...
	vl53l1_dbgmsg("Enter stmvl53l1_power_up_i2c");
	...
	if (data->vdd) {
		...
		vl53l1_info("ldo power is always on, pull up xsdn_gpio");
...
int stmvl53l1_power_down_i2c(void *i2c_object)
{
	...
	vl53l1_dbgmsg("Enter stmvl53l1_power_down_i2c");
...
static int release_reset(struct i2c_data *data)
{
	...
	vl53l1_info("stmvl53l1 release_reset Enter, is_address_change is %d", is_address_change);
	if (is_address_change)
		mutex_lock(&dev_addr_change_mutex);

	gpio_set_value(data->xsdn_gpio, 1);
	gpio_status = gpio_get_value(data->xsdn_gpio);
	vl53l1_info("stmvl53l1 release_reset xsdn_gpio value is %d", gpio_status);
	if (is_address_change) {
...
int stmvl53l1_reset_release_i2c(void *i2c_object)
{
	...
	vl53l1_info("stmvl53l1_reset_release_i2c Enter");
...
int stmvl53l1_reset_hold_i2c(void *i2c_object)
{
	...
	vl53l1_info("stmvl53l1_reset_hold_i2c Enter");

	gpio_set_value(data->xsdn_gpio, 0);

	vl53l1_info("stmvl53l1_reset_hold_i2c End");
...
int stmvl53l1_init_i2c(void)
{
	...
	if (ret)
		vl53l1_errmsg("%d erro ret:%d", __LINE__, ret);
	...
	vl53l1_dbgmsg("End with rc:%d", ret);
...
int stmvl53l1_start_intr(void *object, int *poll_mode)
{
	...
	vl53l1_dbgmsg("to register_irq:%d", i2c_data->irq);
	rc = request_threaded_irq(i2c_data->irq, NULL,
			...
			(void *)i2c_data);
	if (rc) {
		vl53l1_errmsg("fail to req threaded irq rc=%d", rc);
		*poll_mode = 0;
	} else {
		vl53l1_dbgmsg("irq %d now handled", i2c_data->irq);
