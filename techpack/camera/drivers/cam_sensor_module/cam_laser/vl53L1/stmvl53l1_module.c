...
extern ssize_t ap_sensor_route_write(char *buf, size_t count);
...
static struct sensors_classdev sensors_proxi_cdev = {
	...
	.max_range = "ST_PROXIMITY_MAX_RANGE",
	.resolution = "1",
	.sensor_power = "18",
...
{ \
	...
	struct stmvl53l1_data *data = (struct stmvl53l1_data *)o_ctrl->soc_info.soc_private; \
	int param; \
\
...
static int reset_hold(struct stmvl53l1_data *data)
{
	...
	if (data->reset_state) {
		vl53l1_dbgmsg("data->reset_state is (%d)", data->reset_state);
		return 0;
	}

	if (data->force_device_on_en) {
		vl53l1_dbgmsg("data->force_device_on_en is (%d)", data->force_device_on_en);
		return 0;
	}
...
static int stmvl53l1_start(struct stmvl53l1_data *data)
{
	...
	if (rc) {
		vl53l1_errmsg("stmvl53l1_start reset_release fail");
		goto done;
	}
 ...
 * SysFS support
 */
static ssize_t stmvl53l1_show_capture(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct cam_laser_ctrl_t *o_ctrl = dev_get_drvdata(dev);
	struct stmvl53l1_data *data = (struct stmvl53l1_data *)o_ctrl->soc_info.soc_private;
...
static ssize_t stmvl53l1_store_capture(struct device *dev,
				...
				size_t count)
{
	...
	vl53l1_info("enable sensor syntax in %s\n", buf);
	if (val == 1) {
		vl53l1_info("camera ctrl_start");
		data->camera_vote = 1;
		if (!data->enable_sensor)
			rc = ctrl_start(data);
	} else if (val == 2) {
		vl53l1_info("sensor ctrl_start");
		...
		if (!data->sensor_vote && !data->camera_vote) {
			vl53l1_info("camera ctrl_stop");
			rc = ctrl_stop(data);
		}
	} else if (val == 3) {
		data->sensor_vote = 0;
		if (!data->sensor_vote && !data->camera_vote) {
			vl53l1_info("sensor ctrl_stop");
			rc = ctrl_stop(data);
		}
	} else {
...
static ssize_t stmvl53l1_show_chip_enable(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct cam_laser_ctrl_t *o_ctrl = dev_get_drvdata(dev);
	struct stmvl53l1_data *data = (struct stmvl53l1_data *)o_ctrl->soc_info.soc_private;
...
static ssize_t stmvl53l1_store_chip_enable(struct device *dev,
				struct device_attribute *attr, const char *buf,
				size_t count)
{
	vl53l1_dbgmsg("chip_enable is not used");
	return 0;
}

static ssize_t stmvl53l1_show_chip_init(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	vl53l1_dbgmsg("chip_init is not used");
	return 0;
}

static ssize_t stmvl53l1_store_chip_init(struct device *dev,
				struct device_attribute *attr, const char *buf,
				size_t count)
{
	vl53l1_dbgmsg("chip_init is not used");
 ...
 * @ingroup sysfs_attrib
 */
static DEVICE_ATTR(capture, 0664/*S_IWUGO | S_IRUGO*/,
		stmvl53l1_show_capture,
		stmvl53l1_store_capture);
static DEVICE_ATTR(chip_enable, 0664/*S_IWUGO | S_IRUGO*/,
		stmvl53l1_show_chip_enable,
		stmvl53l1_store_chip_enable);
static DEVICE_ATTR(chip_init, 0664/*S_IWUGO | S_IRUGO*/,
		stmvl53l1_show_chip_init,
		stmvl53l1_store_chip_init);
	...
	struct stmvl53l1_ioctl_perform_calibration_t *calib)
{
	...
	if (rc) {
		...
		vl53l1_dbgmsg("VL53L1_PerformXTalkCalibration ok");
	}
...
static int ctrl_perform_calibration(struct stmvl53l1_data *data, void __user *p)
{
	...
	rc = copy_from_user(&calib, (struct stmvl53l1_ioctl_perform_calibration_t *)p, sizeof(calib));
	if (rc) {
		rc = -EFAULT;
		goto done;
	}
											...
											hwlaser_calibration_data_t *p)
{
	...
	if (cal_data.is_read) {
		...
		memset(&(cal_data.u.dataL1.RAW_CalibData), 0, sizeof(hwlaser_calibration_data));
		ret = VL53L1_GetCalibrationData(&data->stdev,
			(VL53L1_CalibrationData_t *)&(cal_data.u.dataL1.RAW_CalibData));
		if (ret) {
			vl53l1_errmsg("VL53L1_GetCalibrationData fail %d", ret);
			ret = store_last_error(data, ret);
			goto done;
		}
		vl53l1_dbgmsg("VL53L1 cross talk is(%u)",cal_data.u.dataL1.RAW_CalibData.algo__xtalk_cpo_HistoMerge_kcps[0]);
		ret = copy_to_user(&(p->u.dataL1.RAW_CalibData), &(cal_data.u.dataL1.RAW_CalibData),
			sizeof(hwlaser_calibration_data));
	} else {
		...
		ret = copy_from_user(&(cal_data.u.dataL1.RAW_CalibData), &(p->u.dataL1.RAW_CalibData),
			sizeof(hwlaser_calibration_data));
		if (ret) {
			vl53l1_errmsg("fail to copy calib data");
			ret = -EFAULT;
...
static int stmvl53l1_set_params(struct stmvl53l1_data *data, void *p)
{
	...
	copy_from_user(&param, (struct stmvl53l1_parameter *)p, sizeof(param));
	if (rc != 0)
		vl53l1_errmsg("copy_from_user failed %d", __LINE__);
	param.status = 0;
...
static int stmvl53l1_laser_get_data(struct stmvl53l1_data *data, void * p)
{
	...
	data->udata.u.data.TimeStamp = rdata->TimeStamp;
	data->udata.u.data.StreamCount = rdata->StreamCount;
	data->udata.u.data.RoiNumber = rdata->RoiNumber;
	data->udata.u.data.NumberOfObjectsFound = rdata->NumberOfObjectsFound;
	data->udata.u.data.RoiStatus = rdata->RoiStatus;
	data->udata.u.data.HasXtalkValueChanged = rdata->HasXtalkValueChanged;
	data->udata.u.data.EffectiveSpadRtnCount = rdata->EffectiveSpadRtnCount;
	data->udata.u.data.DmaxMilliMeter = rdata->DmaxMilliMeter;

	ret = memcpy_s(&(data->udata.u.data.RangeData),
		sizeof(data->udata.u.data.RangeData),
		&rdata->RangeData,
		sizeof(data->udata.u.data.RangeData));
	if (ret != 0)
		vl53l1_errmsg("memcpy failed %d", __LINE__);
	ret = memcpy_s(&(data->udata.u.data.RecommendedDistanceMode),
		sizeof(data->udata.u.data.RecommendedDistanceMode),
		&rdata->RecommendedDistanceMode,
		sizeof(data->udata.u.data.RecommendedDistanceMode));
	if (ret != 0)
		vl53l1_errmsg("memcpy failed %d", __LINE__);

	copy_to_user((hwlaser_RangingData_t *)p, &(data->udata), sizeof(hwlaser_RangingData_t));
	mutex_unlock(&data->work_mutex);
...
static int stmvl53l1_status(struct stmvl53l1_data* data, hwlaser_status_t *laser_status)
{
	...
	put_user(VL53L1_ERROR_NONE, &(laser_status->status));
		...
		void __user *p)
{
	...
	case HWLASER_IOCTL_GET_INFO:
		...
		if (strlen(STMVL53L1_NAME) < HWLASER_NAME_SIZE) {
			strncpy_s(data->pinfo.name, strlen(STMVL53L1_NAME), STMVL53L1_NAME, strlen(STMVL53L1_NAME));
		}
		data->pinfo.i2c_idx = 1;
		data->pinfo.valid = 1;
		data->pinfo.version = HWLASER_ST_L1_VERSION;
		data->pinfo.ap_pos = HWLASER_POS_AP;
		if (strlen(STMVL53L1_NAME) < HWLASER_NAME_SIZE) {
			vl53l1_errmsg("HWLASER_IOCTL_GET_INFO");
	...
	case HWLASER_IOCTL_CALIBRATION_DATA:
		...
		rc = stmvl53l1_laser_get_set_cal_data(data, cal_data);
	break;
...
static void stmvl53l1_input_push_data_singleobject(struct stmvl53l1_data *data)
{
	...
	if (data->sensor_vote) {
		if (meas->RangeStatus == 0) {
			if ((meas->RangeMilliMeter >> 16) > EXT_PROX1_DIS_NEAR_FAR) {
				proxi_status =  EXT_PROX1_DEFAULT_STATUS;
			} else if ((meas->RangeMilliMeter >> 16) < EXT_PROX1_DIS_NEAR_THR) {
				proxi_status = 0;
			}
			/* laser data to proximity sensor */
			vl53l1_dbgmsg("stmvl53l1 single data to proxi, distance %d, status %d", meas->RangeMilliMeter >> 16, meas->RangeStatus);
			...
			st = ap_sensor_route_write((char*)&laser_data_to_proxi,
...
static void stmvl53l1_input_push_data_multiobject(struct stmvl53l1_data *data)
{
	...
	if (data->sensor_vote) {
		if (meas_array[0]->RangeStatus == 0) {
			if ((meas_array[0]->RangeMilliMeter) > EXT_PROX1_DIS_NEAR_FAR) {
				proxi_status =  EXT_PROX1_DEFAULT_STATUS;
			} else if ((meas_array[0]->RangeMilliMeter) < EXT_PROX1_DIS_NEAR_THR) {
				proxi_status = 0;
			}

			/* laser data to proximity sensor */
			vl53l1_dbgmsg("stmvl53l1 multi data to proxi, distance %d, status %d", meas_array[0]->RangeMilliMeter, meas_array[0]->RangeStatus);
			laser_data_to_proxi.sensor_data_head.sensor_type = SENSOR_TYPE_EXT_PROX1;
			laser_data_to_proxi.sensor_data_head.action = STD_SENSOR_DATA_REPORT;
			memcpy(&laser_data_to_proxi.sensor_data[0], &proxi_status, sizeof(int));
 ...
 * @return	0 on success
 */
int stmvl53l1_setup(struct stmvl53l1_data *data, struct cam_laser_ctrl_t *o_ctrl)
{
	int rc = 0;
	VL53L1_DeviceInfo_t dev_info;
	uint8_t model_id = 0;

	vl53l1_info("stmvl53l1_setup Enter");
	...
	if (rc) {
		vl53l1_errmsg("%d,error rc %d", __LINE__, rc);
	...
	if (rc) {
		vl53l1_errmsg("VL53L1_DataInit %d", rc);
		goto exit_unregister_dev_ps;
	}
	vl53l1_info("VL53L1_DataInit %d", rc);

	/* match id */
    rc = VL53L1_RdByte(&data->stdev, VL53L1_IDENTIFICATION__MODEL_ID, &model_id);
    if (rc) {
        vl53l1_errmsg("Vl53l1_RdByte mode id fail %d", rc);
        goto exit_unregister_dev_ps;
    }
	...
	vl53l1_info("o_ctrl->laser_attr_group OK");
	...
	reset_hold(data);
	vl53l1_info("stmvl53l1_setup End");
	return 0;
