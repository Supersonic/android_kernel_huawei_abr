		...
		uint8_t *pdata, uint32_t count)
{
	...
	if (count > STMVL53L1_MAX_CCI_XFER_SZ || count == 0) {
		vl53l1_errmsg("invalid count %d\n", count);
	...
	for (i = 0; (i + 1) <= (count / 23); ++i) {
		state = cci_write(dev, index + i * 23, pdata + i * 23, 23);
	}

	if (count % 23) {
		state = cci_write(dev, index + i * 23, pdata + i * 23, count % 23);
	}
	return state ? VL53L1_ERROR_CONTROL_INTERFACE : VL53L1_ERROR_NONE;
}
		...
		uint8_t *pdata, uint32_t count)
{
	...
	for (i = 0; (i + 1) <= (count / 23); ++i) {
		state = cci_read(dev, index + i * 23, pdata + i * 23, 23);
	}

	if (count % 23) {
		state = cci_read(dev, index + i * 23, pdata + i * 23, count % 23);
	}
