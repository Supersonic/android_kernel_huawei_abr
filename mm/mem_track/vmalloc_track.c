/*
 * mm/mem_track/vmalloc_track.c		vmalloc track function
 *
...
int mm_vmalloc_stack_open(int subtype)
{
	struct vmap_area *va = NULL;
	struct vm_struct *vm = NULL;
	struct list_head *vmap_list = NULL;
...
size_t mm_get_vmalloc_detail(void *buf, size_t len)
{
	...
	for (i = 0; i < ARRAY_SIZE(vm_type) && num--; i++) {
