/*
 * mm/mem_track/slub_track.c   slub trace function
 *
...
static void __slub_del_ringbuf(int type)
{
	...
	/* no need to handle buf spinlock,  protect by slub_track_flag */
	if (srb->buf) {
		vfree(srb->buf);
		srb->buf = NULL;
	}
...
size_t mm_slub_stack_read(struct mm_stack_info *buf, size_t len, int type)
{
	size_t cnt;
	unsigned long flags;
...
static inline void srb_array_lock_init(struct slub_ring_buf *srb)
{
...
static int __init slub_track_init(void)
{
	slub_track_cache = kmem_cache_create("slub_track_cache",
		sizeof(struct slub_rb_node), 0, SLAB_MM_NOTRACE, NULL);
	if (!slub_track_cache) {
	...
	/* we need init srb_array lock before slub_ops exec */
	srb_array_lock_init(&srb_array[SLUB_ALLOC]);
	srb_array_lock_init(&srb_array[SLUB_FREE]);
	return 0;
}
