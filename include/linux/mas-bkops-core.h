...
void mas_bkops_set_status_str(struct mas_bkops *bkops, u32 bkops_stat_max,
			      const char **bkops_stat_str);
int mas_bkops_enable(struct request_queue *q, struct mas_bkops *bkops,
		     struct dentry *pdir);
int mas_bkops_add_debugfs(struct mas_bkops *bkops,
			  const struct dentry *bkops_root);
#ifdef CONFIG_MAS_DEBUG_FS
