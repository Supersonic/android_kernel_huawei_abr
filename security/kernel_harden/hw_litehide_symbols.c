...
#define SYM_NAME_LEN     128
	...
	#define CFI_FUNC_LEN     8
	const char * const CFI_TAIL = ".cfi_jt";
#else
	#define CFI_FUNC_LEN     5
	const char * const CFI_TAIL = ".cfi";
#endif
...
static struct symbol_node *litehide_sym_node_alloc(void)
{
 ...
 */
static struct symbol_node *find_sym_from_blacklist(const char *symbol)
{
	struct symbol_node *node = NULL;
				   ...
				   size_t cnt, loff_t *ppos)
{
	...
	while (cnt > write) {
		...
		(void)strncat(blacklist_cfi, CFI_TAIL, CFI_FUNC_LEN - 1);
		node = litehide_sym_node_alloc();
		if (!node) {
			mutex_unlock(&hidesyms_lock);
			return -ENOMEM;
