...
process_inline:
	...
	if (f2fs_has_inline_data(inode)) {
		...
		if (hmfs_truncate_blocks(inode, 0, false))
			return false;
		goto process_inline;
	}
	return false;
			...
			struct fscrypt_name *fname, struct page **res_page)
{
	...
	namehash = hmfs_dentry_hash(dir, &name, fname);
...
static int f2fs_add_inline_entries(struct inode *dir, void *inline_dentry)
{
	...
	while (bit_pos < d.max) {
		...
		err = hmfs_add_regular_entry(dir, &new_name, NULL, NULL,
...
punch_dentry_pages:
	...
	hmfs_truncate_blocks(dir, 0, false);
				...
				nid_t ino, umode_t mode)
{
	...
	name_hash = hmfs_dentry_hash(dir, new_name, fname);
