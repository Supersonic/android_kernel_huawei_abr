...
static bool f2fs_match_ci_name(const struct inode *dir, const struct qstr *name,
			const u8 *de_name, size_t de_name_len, bool quick)
{
...
static inline bool f2fs_match_name(const struct inode *dir,
		...
		const struct fscrypt_str *cf_str,
		const u8 *de_name, u32 de_name_len)
{
#ifdef CONFIG_UNICODE
	if (hmfs_needs_casefold(dir)) {
			if (cf_str->name) {
					struct qstr cf = {.name = cf_str->name,
									.len = cf_str->len};
					return f2fs_match_ci_name(dir, &cf,
									de_name, de_name_len, true);
			}
			return f2fs_match_ci_name(dir, fname->usr_fname,
							de_name, de_name_len, false);
	}
#endif
	if (fscrypt_match_name(fname, de_name,
			de_name_len))
		return true;
	return false;
}
		...
		struct f2fs_dentry_ptr *d)
{
	...
	while (bit_pos < d->max) {
		...
		if (de->hash_code == namehash && f2fs_match_name(d->inode, fname,
				&cf_str, d->filename[bit_pos], le16_to_cpu(de->name_len)))
			goto found;
					...
					struct page **res_page)
{
	...
	bool room = false;
	int max_slots;
	f2fs_hash_t namehash = hmfs_dentry_hash(dir, &name, fname);
			...
			nid_t ino, umode_t mode)
{
	...
	if (f2fs_has_inline_dentry(dir))
		err = hmfs_add_inline_entry(dir, &new_name, fname,
						dentry, inode, ino, mode);
	dentry_hash = hmfs_dentry_hash(dir, &new_name, fname);
	if (err == -EAGAIN)
		err = hmfs_add_regular_entry(dir, &new_name, fname->usr_fname,
						dentry, dentry_hash, inode, ino, mode);
					...
					struct inode *dir, struct inode *inode)
{
	...
	if (bit_pos == NR_DENTRY_IN_BLOCK &&
		...
		hmfs_clear_page_cache_dirty_tag(page);
