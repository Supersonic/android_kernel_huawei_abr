...
int hmfs_truncate(struct inode *inode)
{
	...
	err = hmfs_truncate_blocks(inode, i_size_read(inode), true);
	if (err)
		return err;
...
static int f2fs_collapse_range(struct inode *inode, loff_t offset, loff_t len)
{
	...
	ret = hmfs_truncate_blocks(inode, new_size, true);
	up_write(&F2FS_I(inode)->i_mmap_sem);
	if (!ret)
...
static int f2fs_insert_range(struct inode *inode, loff_t offset, loff_t len)
{
	...
	down_write(&F2FS_I(inode)->i_mmap_sem);
	ret = hmfs_truncate_blocks(inode, i_size_read(inode), true);
	up_write(&F2FS_I(inode)->i_mmap_sem);
	if (ret)
		return ret;
...
static int __f2fs_ioc_setflags(struct inode *inode, unsigned int flags)
{
	...
	if ((flags ^ oldflags) & F2FS_CASEFOLD_FL) {
		if (!f2fs_sb_has_casefold(F2FS_I_SB(inode)->sb))
			return -EOPNOTSUPP;
