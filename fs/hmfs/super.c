					...
					bool is_remount)
{
	...
	if (!f2fs_sb_has_encrypt(sbi->sb)) {
		hmfs_msg(sbi->sb, KERN_ERR, "Encrypt feature is off");
		return -EINVAL;
	}
...
static int parse_options(struct super_block *sb, char *options, bool is_remount)
{
...
#else
			hmfs_msg(sb, KERN_INFO, "inline encryption not supported");
#endif
			break;
		case Opt_checkpoint:
			name = match_strdup(&args[0]);
	...
	if (f2fs_sb_has_casefold(sbi->sb)) {
		hmfs_msg(sb, KERN_ERR,
...
{
#ifdef CONFIG_UNICODE
	if (f2fs_sb_has_casefold(sbi->sb) && !sbi->sb->s_encoding) {
		...
		hmfs_msg(sbi->sb, KERN_ERR, "Using encoding defined by superblock: "
...
#else
	if (f2fs_sb_has_casefold(sbi->sb)) {
		hmfs_msg(sbi->sb, KERN_INFO, "Filesystem with casefold feature cannot be mounted without CONFIG_UNICODE");
		return -EINVAL;
	}
#endif
	return 0;
...
#endif
#endif /*end of CONFIG_FS_UNI_ENCRYPTION*/
...
{
	if (bioset_init(&hmfs_bioset, F2FS_BIO_POOL_SIZE,
				offsetof(struct hmfs_bio, bio),
				BIOSET_NEED_BVECS))
		return -ENOMEM;
	return 0;
}
